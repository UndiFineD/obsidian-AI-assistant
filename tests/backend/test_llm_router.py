...
class TestHybridLLMRouter:
    """Test suite for the HybridLLMRouter class."""
    ...
    @pytest.fixture
    def mock_gpt4all(self):
        """Mock GPT4All model."""
        with patch('llm_router.GPT4All') as mock_gpt4all_class:
    ...
    @pytest.fixture
    def router_with_mocks(self, mock_llama, mock_gpt4all):
        """Create HybridLLMRouter with mocked models."""
    ...
    def test_router_initialization_default_values(self):
    ...
    def test_router_initialization_custom_values(self):
    ...
    def test_model_loading_llama_available(self, mock_llama):
        """Test model loading when LLaMA is available."""
        with patch('os.path.exists', return_value=True):
            router = HybridLLMRouter(llama_model_path="fake/llama.bin")
            
            # LLaMA should be loaded
            assert router.llama_model is not None
            assert router.available_models["llama"] is True
    
    def test_model_loading_gpt4all_available(self, mock_gpt4all):
        """Test model loading when GPT4All is available."""
        with patch('os.path.exists', return_value=True), \
             patch('llm_router.Llama', None):  # LLaMA not available
            
            router = HybridLLMRouter(gpt4all_model_path="fake/gpt4all.bin")
            
            # GPT4All should be loaded
            assert router.gpt4all_model is not None
            assert router.available_models["gpt4all"] is True

...
def mock_log_manager(temp_log_dir):
    ...
    mock_manager.get_log_stats = Mock(return_value={
        'total_logs': 1000,
        'error_count': 50,
        'warning_count': 100,
    })
        ...
        with patch('backend.log_management.log_audit'):
            with patch('backend.log_management.log_security'):
                with patch('backend.log_management.logger'):
                    ...
                    with patch('backend.log_management.asyncio.sleep', new_callable=AsyncMock):
...
def test_search_logs_by_search_term(client):
    ...
    response = client.post(
        "/api/logs/search",
        json={"search_term": "Application started"}
    )
    ...
    assert any("Application started" in r["message"] or "Application started" in r["raw_line"] 
...
def test_export_logs_json(client, temp_log_dir):
    ...
    response = client.post(
        "/api/logs/export?format_type=json&compress=false"
    )
...
def test_export_logs_csv(client):
    ...
    response = client.post(
        "/api/logs/export?format_type=csv&compress=false"
    )
...
def test_export_logs_text(client):
    ...
    response = client.post(
        "/api/logs/export?format_type=text&compress=false"
    )
...
def test_export_logs_compressed(client):
    ...
    response = client.post(
        "/api/logs/export?format_type=json&compress=true"
    )
...
def test_cleanup_protects_active_logs(client):
    ...
    response = client.post("/api/logs/cleanup?days=30")  # Use 30 instead of 0 to avoid validation error
...
def test_matches_filter_time_range():
    ...
    filter_params = LogFilter(
        start_time=datetime(2025, 10, 17, 11, 0, 0)
    )

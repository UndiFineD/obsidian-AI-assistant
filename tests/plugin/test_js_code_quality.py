...
class TestJavaScriptCodeQuality:
    ...
    def test_js_syntax_validity(self, js_files):
        ...
        for js_file in js_files:
            try:
                ...
                assert result.returncode == 0, f"JavaScript file {js_file.name} has syntax errors: {result.stderr}"
        ...
        for js_file in js_files:
            ...
            for line_num, line in enumerate(lines, 1):
                if line.strip():  # Skip empty lines
                    ...
                    assert '\t' not in line, f"File {js_file.name} line {line_num} contains tabs instead of spaces"
                    
                    # Check for consistent indentation (multiples of 4 spaces)
                    leading_spaces = len(line) - len(line.lstrip())
                    if leading_spaces > 0:
                        assert leading_spaces % 4 == 0, f"File {js_file.name} line {line_num} has inconsistent indentation ({leading_spaces} spaces)"
    ...
    def test_consistent_quotes(self, js_files):
        ...
        for js_file in js_files:
            ...
            if single_quotes > 0 or double_quotes > 0:
                ...
                print(f"Quote usage in {js_file.name}: {single_quotes} single, {double_quotes} double ({single_ratio:.2f} single ratio)")
    ...
    def test_class_definitions(self, js_files):
        ...
        for js_file in js_files:
            ...
            for class_name in class_matches:
                ...
                assert class_name[0].isupper(), f"Class {class_name} in {js_file.name} should start with uppercase"
                assert re.match(r'^[A-Z][a-zA-Z0-9]*$', class_name), f"Class {class_name} in {js_file.name} should be PascalCase"
    ...
    def test_function_definitions(self, js_files):
        ...
        for js_file in js_files:
            ...
            function_matches = re.findall(r'(?:function\s+|async\s+)?(\w+)\s*\([^)]*\)\s*{', content)
            ...
            for func_name in function_matches:
                ...
                assert func_name[0].islower(), f"Function {func_name} in {js_file.name} should start with lowercase"
    ...
    def test_console_log_usage(self, js_files):
        ...
        for js_file in js_files:
            ...
            if lines_count > 20:  # Only check for files with substantial content
                ...
                assert ratio < 0.05, f"File {js_file.name} has too many console.log statements ({console_logs} in {lines_count} lines)"
        ...
        for js_file in js_files:
            ...
            if lines_count > 100:  # Only check very substantial files (raised threshold)
                ...
                if try_catch_count == 0:
                    print(f"Warning: {js_file.name} might benefit from error handling (try-catch blocks)")
    ...
    def test_module_exports(self, js_files):
        ...
        for js_file in js_files:
            ...
            if has_classes or has_functions:
                assert has_exports, f"File {js_file.name} defines classes/functions but doesn't export them"
...
class TestJavaScriptFunctionality:
    ...
    def test_backend_client_structure(self, plugin_dir):
    ...
    def test_voice_functionality(self, plugin_dir):
        ...
        for file_name in voice_files:
            ...
            if 'voice.js' in file_name:
                assert 'mediaRecorder' in content or 'MediaRecorder' in content, "voice.js should handle media recording"
            
            # Check for error handling patterns (more flexible)
            has_error_handling = (
                'try' in content or 'catch' in content or 
                'error' in content or 'Error' in content or
                'console.log' in content or 'console.error' in content
            )
            assert has_error_handling, f"{file_name} should have some form of error handling or logging"
        ...
        for js_file in js_files:
            ...
            for pattern in secret_patterns:
                ...
                for match in matches:
                    ...
                    if any(word in match for word in ['error', 'console', 'log', 'message', 'debug']):
                        continue  # Skip error handling patterns
                    
                    print(f"Warning: Potential hardcoded credential in {js_file.name}: {match[:50]}...")
...
def run_js_tests():
    ...
    test_files = [
        'tests/plugin/test_js_code_quality.py'
    ]

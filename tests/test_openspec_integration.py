...
class TestOpenSpecIntegration:
    ...
    def test_generated_changes_exist_and_have_structure(self):
        ...
        for change_dir in changes[:sample_size]:
            ...
            assert len(proposal_content.strip()) > 50, f"Proposal too short in {change_dir.name}"
            ...
            assert f"# Tasks: {change_dir.name}" in tasks_content, (
    ...
    def test_change_id_naming_convention(self):
        ...
        for change_dir in changes_dir.iterdir():
            if change_dir.is_dir() and change_dir.name not in (
                ...
                is_legacy_format = change_id.startswith("update-doc-") or change_id.startswith(
                    "update-spec-"
                )
    ...
    def test_proposal_format_compliance(self):
        ...
        for change_dir in changes[:sample_size]:
            ...
            if proposal_file.exists():
                ...
                assert f"# Change Proposal: {change_dir.name}" in content, (
        ...
        for change_dir in changes_dir.iterdir():
            if change_dir.is_dir() and change_dir.name not in ("archive", "test-step1"):
                ...
                if spec_file.exists():
                    ...
                    assert expected_title in content, f"Wrong spec delta title in {change_dir.name}"
                    ...
                    assert "#### Scenario:" in content, f"Missing scenario in {change_dir.name}"
    ...
    def test_coverage_of_important_docs(self):
        ...
        for doc in important_docs:
            ...
            has_change = any(change_id in existing_changes for change_id in possible_change_ids)
            if has_change:
            ...
            pytest.skip("No changes found for tested important docs (might be expected)")
    ...
    def test_change_proposals_are_actionable(self):
        ...
        for change_dir in changes[:sample_size]:
            ...
            if proposal_file.exists():
                ...
                assert len(content.strip()) > 200, f"Proposal in {change_dir.name} seems too short"

                # Should mention specific files or components
                backtick_mentions = content.count("`")
                assert backtick_mentions >= 2, (

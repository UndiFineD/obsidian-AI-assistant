...
class AsyncTestRunner:
    ...
    def cleanup_test_directories(self):
        ...
        print(
            f"{Colors.colorize('🧹 CLEANING UP TEST DIRECTORIES', Colors.BOLD + Colors.YELLOW)}"
        )
        print("=" * 60)
        ...
        for dir_name in dirs_to_remove:
            ...
            if dir_path.exists() and dir_path.is_dir():
                try:
                    ...
                    print(
                        f"  {Colors.colorize('⚠️ Could not remove directory:', Colors.YELLOW)} {dir_name} - {e}"
                    )
        ...
        for file_path in files_to_remove:
            ...
            if full_path.exists() and full_path.is_file():
                try:
                    ...
                    print(
                        f"  {Colors.colorize('⚠️ Could not remove file:', Colors.YELLOW)} {file_path} - {e}"
                    )
    ...
    def discover_and_categorize_tests(self) -> List[TestFile]:
        ...
        print(
            f"{Colors.colorize('🔍 DISCOVERING AND CATEGORIZING TESTS', Colors.BOLD + Colors.CYAN)}"
        )
        print("=" * 60)
        ...
        categories = {
            "Core Backend": {
                "priority": 1,
                "patterns": ["backend/test_backend.py", "backend/test_settings.py"],
            },
            "Model Management": {
                "priority": 2,
                "patterns": ["backend/test_modelmanager*.py"],
            },
            "Embeddings & Search": {
                "priority": 3,
                "patterns": [
            ...
            "Caching & Storage": {
                "priority": 4,
                "patterns": ["backend/test_caching*.py"],
            },
            "Security & Config": {
                "priority": 5,
                "patterns": ["backend/test_security.py", "backend/test_config*.py"],
            },
            ...
            "Integration Tests": {
                "priority": 10,
                "patterns": ["integration/test_*.py"],
            },
            "Enterprise Tests": {
                "priority": 11,
                "patterns": ["integration/test_enterprise*.py"],
            },
            "Performance Tests": {"priority": 12, "patterns": ["test_performance.py"]},
            "Utility & Server": {
                "priority": 13,
                "patterns": ["test_server.py", "run_tests.py"],
            },
        ...
        for test_file in all_test_files:
            ...
            for cat_name, cat_info in categories.items():
                for pattern in cat_info["patterns"]:
                    if (
                        ...
                        or str(rel_path).replace("\\", "/") == pattern
                    ):
                        category = cat_name
            ...
            print(
                f"  {Colors.colorize(f'[{category}]', Colors.BLUE)} {rel_path} "
            ...
            for line in lines:
                ...
                while indent_stack and indent <= indent_stack[-1][1]:
                    ...
                    if indent_stack:
                        current_class = (
                            indent_stack[-1][0]
                            if isinstance(indent_stack[-1][0], str)
                            and indent_stack[-1][0].startswith("Test")
                            else None
                        )
                    else:
                        current_class = None
    ...
    def print_header(self, test_files: List[TestFile]):
        ...
        print(
            Colors.colorize(
                "🧪 OBSIDIAN AI ASSISTANT - COMPREHENSIVE ASYNC TEST SUITE",
        ...
        print("🔧 Configuration:")
        print(
            f"   • Max Workers: {Colors.colorize(str(self.max_workers), Colors.GREEN)}"
        )
        print(f"   • Test Files: {Colors.colorize(str(len(test_files)), Colors.GREEN)}")
        print(
            f"   • Total Tests: {Colors.colorize(str(total_functions), Colors.GREEN)}"
        )
        print(
            f"   • Estimated Time: {Colors.colorize(f'{total_estimated_time:.1f}s', Colors.BLUE)}"
        )
        print(
    ...
    def print_test_status(self, test_result: TestResult, total_tests: int):
        ...
        status_str = Colors.colorize(
            f"{symbol} {test_result.status.value}", status_color
        )

        # Format category
        category_str = Colors.colorize(
            f"[{test_result.category}]", Colors.DIM + Colors.BLUE
        )
        ...
        worker_colored = (
            Colors.colorize(worker_str, Colors.DIM + Colors.MAGENTA)
            if worker_str
            else ""
        )
        ...
        line = (
            f"[{timestamp}] {counter_str} {category_str:<20} {display_name} "
    ...
    async def run_test_file_async(
        self, test_file: TestFile, worker_id: str
    ) -> List[TestResult]:
        ...
        for test_func in test_file.test_functions:
            ...
            try:
                ...
                result = await loop.run_in_executor(
                    self.executor, self._run_subprocess, cmd
                )
    ...
    async def run_all_tests_async(self, test_files: List[TestFile]) -> Dict:
        ...
        print(
            f"{Colors.colorize('🚀 STARTING ASYNC TEST EXECUTION', Colors.BOLD + Colors.GREEN)}"
        )
        print(
            f"Running {len(test_files)} test files with {self.max_workers} workers...\n"
        )
    ...
    def print_comprehensive_summary(self):
        ...
        print(
            Colors.colorize(
                "📊 COMPREHENSIVE ASYNC TEST SUMMARY", Colors.BOLD + Colors.CYAN
            )
        )
        print("=" * 100)
        ...
        print(
            f"🚀 Speedup Factor: {Colors.colorize(f'{total_test_time/total_duration:.1f}x', Colors.GREEN)}"
        )
        print(f"🧪 Total Tests: {Colors.colorize(str(total_tests), Colors.BOLD)}")
        print(
            f"👥 Workers Used: {Colors.colorize(str(self.max_workers), Colors.MAGENTA)}"
        )
        print(
        ...
        for category, stats in sorted(category_stats.items()):
            ...
            if total_cat > 0:
                ...
                status_color = (
                    Colors.GREEN
                    if cat_pass_rate >= 90
                    else Colors.YELLOW if cat_pass_rate >= 70 else Colors.RED
                )
                print(
                    f"  📁 {category:<20} {Colors.colorize(f'{cat_pass_rate:5.1f}%', status_color)} "
        ...
        if self.results:
            slowest_tests = sorted(
                self.results, key=lambda x: x.duration, reverse=True
            )[:10]
            ...
            for i, result in enumerate(slowest_tests, 1):
                duration_color = (
                    ...
                    else Colors.YELLOW if result.duration > 2 else Colors.GREEN
                )
                print(
                    f"  {i:2d}. {result.name:<35} {Colors.colorize(f'{result.duration:.3f}s', duration_color)} [{result.category}]"
                )

            # Fastest tests
            fastest_tests = sorted(
                [r for r in self.results if r.status == TestStatus.PASSED],
            ...
            if fastest_tests:
                ...
                for i, result in enumerate(fastest_tests, 1):
                    print(
                        f"  {i}. {result.name:<35} {Colors.colorize(f'{result.duration:.3f}s', Colors.GREEN)} [{result.category}]"
                    )
        ...
        if failed_tests:
            ...
            for result in failed_tests[:10]:  # Show max 10 failures
                ...
                if result.error_message and len(result.error_message) < 200:
                    ...
                    print(
                        f"       {Colors.colorize(error_preview + '...', Colors.DIM + Colors.RED)}"
                    )
        ...
        if (
            ...
            print(
                Colors.colorize(
                    "🌟 GREAT SUCCESS - 90%+ PASS RATE! 🌟", Colors.BOLD + Colors.YELLOW
                )
            )
        else:
            print(
                Colors.colorize(
                    "💥 SOME TESTS FAILED - NEEDS ATTENTION", Colors.BOLD + Colors.RED
                )
            )
    ...
    def generate_summary_dict(self) -> Dict:
        ...
        return {
            ...
            "speedup_factor": sum(r.duration for r in self.results)
...
async def main():
    ...
    print(
        Colors.colorize(
            "🚀 INITIALIZING COMPREHENSIVE ASYNC TEST RUNNER", Colors.BOLD + Colors.CYAN
        )
    )
    ...
    try:
        ...
        print(
            f"📄 Detailed results saved to: {Colors.colorize(str(results_file), Colors.BLUE)}"
        )

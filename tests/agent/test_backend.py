...
from agent.backend import AskRequest, ReindexRequest, WebRequest
...
class TestBackendAPI:
    ...
    #     with patch('agent.agent.model_manager') as mock_model, \
    #         patch('agent.agent.emb_manager') as mock_emb, \
    #         patch('agent.agent.vault_indexer') as mock_vault, \
    #         patch('agent.agent.cache_manager') as mock_cache, \
    #         patch('agent.agent.get_cache_manager') as mock_performance_cache:
    ...
    def mock_web_services(self):
        with patch("agent.agent.vault_indexer") as mock_vault, patch(
...
class TestConfigAndPerformanceEndpoints:
    ...
    def test_get_config_contains_expected_fields(self, client):
        ...
        data = resp.json()
        assert "agent/vector_db" in data

    def test_post_reload_config_success(self, client):
        with patch("agent.agent.reload_settings") as mock_reload, patch(
            "agent.agent._settings_to_dict", return_value={"api_port": 8000}
        ):
    ...
    def test_post_reload_config_failure(self, client):
        with patch("agent.agent.reload_settings", side_effect=RuntimeError("boom")):
            r = client.post("/api/config/reload", json={})
    ...
    def test_post_update_config_success(self, client):
        with patch("agent.agent.update_settings") as mock_update, patch(
            "agent.agent._settings_to_dict", return_value={"api_port": 8001}
        ):
    ...
    def test_performance_metrics_success(self, client):
        with patch(
            "agent.agent.PerformanceMonitor.get_system_metrics",
            return_value={"cpu": 10},
        ):
    ...
    def test_cache_stats_and_clear(self, client):
        ...
        with patch("agent.agent.get_cache_manager", return_value=DummyCache()):
    ...
    def test_performance_optimize_success(self, client):
        ...
        with patch("agent.agent.get_task_queue", side_effect=fake_get_task_queue):
...
class TestSearchAndIndexingEndpoints:
    ...
    def test_search_success(self, client):
        with patch("agent.agent.emb_manager") as mock_emb:
    ...
    def test_transcribe_invalid_audio(self, client):
        with patch(
            "agent.agent.validate_base64_audio",
    ...
    def test_index_pdf_success(self, client):
        ...
        with patch("agent.agent.validate_pdf_path", return_value=valid), patch(
            "agent.agent.vault_indexer"
        ) as mock_vault:
...
class TestServiceIntegration:
    ...
    def test_service_initialization(self, mock_env_vars):
        ...
        with patch("agent.agent.ModelManager") as MockModel, patch(
            "agent.agent.EmbeddingsManager"
        ), patch("agent.agent.CacheManager"):
            # Explicitly instantiate ModelManager to trigger the mock
            from agent.backend import ModelManager
...
class TestOpenSpecAndSecurityEndpoints:
    ...
    def test_get_security_status(self, client):
        with patch("agent.agent.get_advanced_security_config") as mock_sec:

...
class TestCompleteWorkflows:
    ...
    def test_client_with_real_cache(self):
        ...
        with patch('backend.backend.init_services'):
            with patch('backend.backend.model_manager') as mock_model, \
                ...
                mock_model.generate.side_effect = lambda prompt, **kwargs: f"AI response to: {prompt[:50]}..."
                ...
                mock_vault.index_vault.return_value = ["note1.md", "note2.md", "note3.md"]
    ...
    def test_error_recovery_workflow(self, test_client_with_real_cache):
        ...
        assert response.status_code in [200, 400, 422], f"Expected error or success for {endpoint}, but got {response.status_code}"

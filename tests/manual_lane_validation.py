        ...
        try:
            result = subprocess.run(
                ["python", "scripts/workflow.py",
                 "--change-id", "test-13-docs-lane",
                 "--title", "TEST-13: Docs Lane Validation",
                 "--owner", "automation",
                 "--lane", "docs",
                 "--dry-run"],
                capture_output=True,
            ...
            print(f"  • Lane Detection: {'✅ Found' if 'docs' in result.stdout.lower() else '❌ Not found'}")
...
""")
        ...
        try:
            result = subprocess.run(
                ["python", "scripts/workflow.py",
                 "--change-id", "test-14-standard-lane",
                 "--title", "TEST-14: Standard Lane Validation",
                 "--owner", "automation",
                 "--lane", "standard",
                 "--dry-run"],
                capture_output=True,
            ...
            success = (
                result.returncode == 0 and
                elapsed_time < 900 and
                "standard" in result.stdout.lower()
            )
            ...
            print(f"  • Return Code: {result.returncode} (expected: 0)")
            print(f"  • Lane Detection: {'✅ Found' if 'standard' in result.stdout.lower() else '❌ Not found'}")
...
""")
        ...
        try:
            result = subprocess.run(
                ["python", "scripts/workflow.py",
                 "--change-id", "test-15-heavy-lane",
                 "--title", "TEST-15: Heavy Lane Validation",
                 "--owner", "automation",
                 "--lane", "heavy",
                 "--dry-run"],
                capture_output=True,
            ...
            success = (
                result.returncode == 0 and
                elapsed_time < 1200 and
                "heavy" in result.stdout.lower()
            )
            ...
            print(f"  • Return Code: {result.returncode} (expected: 0)")
            print(f"  • Lane Detection: {'✅ Found' if 'heavy' in result.stdout.lower() else '❌ Not found'}")

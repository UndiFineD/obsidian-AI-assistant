...
def test_phase1_execution():
    ...
    try:
        ...
        record_test(
            "phase1_lane_mapping_added",
            "PASS",
            "LANE_MAPPING added to workflow.py"
        )
...
def test_phase2_execution():
    ...
    try:
        ...
        if "class QualityGates" not in content:
            record_test(
                "phase2_qualitygates_class",
                "FAIL",
                "QualityGates class not defined"
            )
            return False
...
def test_phase3_execution():
    ...
    try:
        ...
        status_file = implement.project_root / "openspec" / "changes" / "workflow-improvements" / "status.json"
        ...
        record_test(
            "phase3_json_valid",
            "PASS",
            "status.json contains valid JSON"
        )
...
def test_integration_all_phases():
    ...
    try:
        ...
        checks = [
            (implement.scripts_root / "workflow.py", "LANE_MAPPING", "Lane selection in workflow.py"),
            (implement.scripts_root / "quality_gates.py", "class QualityGates", "QualityGates module"),
            (implement.project_root / "openspec" / "changes" / "workflow-improvements" / "status.json", None, "Status tracking template"),
        ]

...
def test_file_exists(
    file_path: Path, description: str, category: str = "Structure"
) -> bool:
...
def test_content_matches(
    file_path: Path, pattern: str, description: str, category: str = "Content"
) -> bool:
...
def test_json_schema(
    file_path: Path, schema_keys: List[str], description: str, category: str = "JSON"
) -> bool:
    ...
    try:
        ...
        if missing_keys:
            ...
            _record_fail(
                description, f"Missing required keys: {missing_keys}", category
            )
            return False
        else:
            _record_pass(
                description, f"All required keys present: {schema_keys}", category
            )
            return True
...
def test_file_has_section(
    file_path: Path, section_pattern: str, description: str, category: str = "Sections"
) -> bool:
...
def test_line_count(
    file_path: Path, min_lines: int, description: str, category: str = "Completeness"
) -> bool:
    ...
    if line_count >= min_lines:
        _record_pass(
            description,
            f"File has {line_count} lines (required: {min_lines})",
            category,
        )
        return True
    else:
...
def _record_pass(test_name: str, message: str, category: str):
    ...
    test_results["tests"].append(
        {"name": test_name, "category": category, "result": "PASS", "message": message}
    )
...
def _record_fail(test_name: str, message: str, category: str):
    ...
    test_results["tests"].append(
        {"name": test_name, "category": category, "result": "FAIL", "message": message}
    )
...
def _record_skip(test_name: str, message: str, category: str):
    ...
    test_results["tests"].append(
        {"name": test_name, "category": category, "result": "SKIP", "message": message}
    )
...
def test_implement_lane_selection_logic():
    ...
    try:
        ...
        if "LANE_MAPPING = {" in impl_content:
            ...
            _record_pass(
                "LANE_MAPPING exists", "LANE_MAPPING dict found", "Implementation"
            )
        else:
            _log_test("  ✗ LANE_MAPPING dictionary NOT defined", "Implementation")
            _record_fail(
                "LANE_MAPPING exists", "LANE_MAPPING dict not found", "Implementation"
            )
            return
        ...
        for lane in lanes:
            if f'"{lane}"' in impl_content or f"'{lane}'" in impl_content:
                ...
                _record_pass(
                    f"Lane {lane}",
                    f"Lane '{lane}' found in LANE_MAPPING",
                    "Implementation",
                )
            else:
                _record_fail(
                    f"Lane {lane}", f"Lane '{lane}' not found", "Implementation"
                )

        # Verify docs lane stages are correct
        if (
            '"stages": [0, 2, 3, 4, 9, 10, 11, 12]' in impl_content
            or "'stages': [0, 2, 3, 4, 9, 10, 11, 12]" in impl_content
        ):
            _log_test(
                "  ✓ Docs lane stages correct (0,2,3,4,9,10,11,12)", "Implementation"
            )
            _record_pass(
                "Docs lane stages",
                "Stages [0,2,3,4,9,10,11,12] found",
                "Implementation",
            )
        else:
            _record_fail(
                "Docs lane stages",
                "Stages not matching expected [0,2,3,4,9,10,11,12]",
                "Implementation",
            )
        ...
        if "def should_run_quality_gates" in impl_content:
            _log_test(
                "  ✓ should_run_quality_gates() function defined", "Implementation"
            )
            _record_pass(
...
def test_implement_quality_gates_logic():
    ...
    try:
        ...
        if "class QualityGates" in impl_content:
            ...
            _record_pass(
                "QualityGates class", "Class definition found", "Implementation"
            )
        else:
        ...
        if "THRESHOLDS = {" in impl_content:
            ...
            if '"docs": {' in impl_content and '"enabled": False' in impl_content:
                ...
                _record_pass(
                    "Docs lane thresholds", "Docs lane disabled", "Implementation"
                )

            if (
                '"standard": {' in impl_content
                and '"pytest_pass_rate": 0.80' in impl_content
            ):
                _log_test(
                    "  ✓ Standard lane has 80% pass rate threshold", "Implementation"
                )
                _record_pass(
                    "Standard lane thresholds", "80% pass rate found", "Implementation"
                )

            if (
                '"heavy": {' in impl_content
                and '"pytest_pass_rate": 1.0' in impl_content
                and '"coverage_minimum": 0.85' in impl_content
            ):
                _log_test(
                ...
                _record_pass(
                    "Heavy lane thresholds", "Strict thresholds found", "Implementation"
                )
        else:
            _record_fail(
                "THRESHOLDS config", "Configuration not found", "Implementation"
            )
...
def test_implement_status_json_logic():
    ...
    try:
        ...
        if "def create_status_json_template" in impl_content:
            _log_test(
                "  ✓ create_status_json_template() function defined", "Implementation"
            )
            _record_pass("Status template function", "Function found", "Implementation")
        else:
            _record_fail(
                "Status template function", "Function not found", "Implementation"
            )
            return
        ...
        for field in required_fields:
            if f'"{field}"' in impl_content or f"'{field}'" in impl_content:
                ...
                _record_pass(
                    f"Field {field}", "Field found in template", "Implementation"
                )
            else:
                _record_fail(
                    f"Field {field}", f"Field not found in template", "Implementation"
                )
        ...
        if "resumable" in impl_content and "resume_from_stage" in impl_content:
            ...
            _record_pass(
                "Resumption support",
                "Resumable and resume_from_stage found",
                "Implementation",
            )
        else:
            _record_fail(
                "Resumption support", "Resumption fields not found", "Implementation"
            )
    ...
    try:
        ...
        for phase_func, phase_name in phases:
            if phase_func in impl_content:
                ...
                _record_pass(
                    f"{phase_name} call", f"{phase_func} called", "Implementation"
                )
            else:
                _record_fail(
                    f"{phase_name} call", f"{phase_func} not called", "Implementation"
                )
...
def test_generated_files_exist():
    ...
    try:
        ...
        if "scripts/workflow.py" in impl_content and "LANE_MAPPING" in impl_content:
            ...
            _record_pass(
                "workflow.py generation",
                "File modification logic found",
                "Implementation",
            )
        else:
            _record_fail(
                "workflow.py generation",
                "Modification logic not found",
                "Implementation",
            )

        # Check for quality_gates.py creation
        if (
            "scripts/quality_gates.py" in impl_content
            and "QualityGates" in impl_content
        ):
            _log_test("  ✓ quality_gates.py creation referenced", "Implementation")
            _record_pass(
                "quality_gates.py generation", "Creation logic found", "Implementation"
            )
        else:
            _record_fail(
                "quality_gates.py generation",
                "Creation logic not found",
                "Implementation",
            )
        ...
        if "status.json" in impl_content and "template" in impl_content.lower():
            ...
            _record_pass(
                "status.json generation", "Template logic found", "Implementation"
            )
        else:
            _record_fail(
                "status.json generation", "Template logic not found", "Implementation"
            )
...
def main() -> int:
    ...
    test_file_exists(
        change_root / "proposal.md", "Proposal document exists", "Documentation"
    )
    test_line_count(
        change_root / "proposal.md",
        800,
        "Proposal has comprehensive content (800+ lines)",
        "Documentation",
    )
    test_file_has_section(
        change_root / "proposal.md",
        r"## Why",
    ...
    test_file_has_section(
        change_root / "proposal.md",
        r"## Objectives",
        "Proposal has 'Objectives' section",
        "Documentation",
    )
    test_file_has_section(
        change_root / "proposal.md",
        r"## Appendices",
        "Proposal has 'Appendices' section",
        "Documentation",
    )
    ...
    test_file_exists(
        change_root / "spec.md", "Specification document exists", "Documentation"
    )
    test_line_count(
        change_root / "spec.md",
        1000,
        "Spec has comprehensive content (1000+ lines)",
        "Documentation",
    )
    test_file_has_section(
        change_root / "spec.md",
        r"## 3\. Acceptance Criteria",
        "Spec has 'Acceptance Criteria'",
        "Documentation",
    )
    test_file_has_section(
        change_root / "spec.md",
        r"## 4\. Technical Design",
        "Spec has 'Technical Design'",
        "Documentation",
    )
    test_file_has_section(
        change_root / "spec.md",
        r"## 7\. Data Models",
        "Spec has 'Data Models'",
        "Documentation",
    )
    ...
    test_line_count(
        ...
        1200,
        "Tasks has comprehensive breakdown (1200+ lines)",
        "Documentation",
    )
    test_file_has_section(
        change_root / "tasks.md",
        r"## Implementation Tasks",
        "Tasks has 'Implementation Tasks'",
        "Documentation",
    )
    test_file_has_section(
        change_root / "tasks.md",
        r"## Testing Tasks",
        "Tasks has 'Testing Tasks'",
        "Documentation",
    )
    ...
    test_file_exists(
        change_root / "test_plan.md", "Test plan document exists", "Documentation"
    )
    test_line_count(
        change_root / "test_plan.md",
        800,
        "Test plan has comprehensive coverage (800+ lines)",
        "Documentation",
    )
    test_file_has_section(
        change_root / "test_plan.md",
        r"## 1\. Test Strategy",
        "Test plan has 'Test Strategy'",
        "Documentation",
    )
    test_file_has_section(
        change_root / "test_plan.md",
        r"## 6\. Unit Testing",
        "Test plan has 'Unit Testing'",
        "Documentation",
    )
    ...
    test_file_has_section(
        change_root / "todo.md",
        r"## Task List|## Summary",
        "Todo has task tracking sections",
        "Documentation",
    )
    ...
    test_content_matches(
        change_root / "spec.md",
        r"--lane\s+\[docs\|standard\|heavy\]",
        "Spec documents --lane flag syntax",
        "Lane Selection",
    )
    test_content_matches(
        change_root / "spec.md",
        r"auto.?detect.*code.*changes",
        "Spec documents auto-detection of code changes",
        "Lane Selection",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"Lane.*to.*Stage.*Mapping",
        "Proposal has Lane-to-Stage mapping",
        "Lane Selection",
    )
    ...
    test_content_matches(
        ...
        "Spec documents ThreadPoolExecutor parallelization",
        "Parallelization",
    )
    test_content_matches(
        change_root / "spec.md",
        r"Stages\s+2.?6.*parallel",
        "Spec defines parallel stages (2-6)",
        "Parallelization",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"--no-parallel",
        "Proposal documents --no-parallel flag",
        "Parallelization",
    )
    ...
    test_content_matches(
        change_root / "spec.md",
        r"scripts/quality_gates\.py",
        "Spec references quality_gates.py module",
        "Quality Gates",
    )
    test_content_matches(
        change_root / "spec.md",
        r"ruff.*mypy.*pytest.*bandit",
        "Spec lists all 4 quality gate tools (ruff, mypy, pytest, bandit)",
        "Quality Gates",
    )
    test_content_matches(
        change_root / "spec.md",
        r"quality_metrics\.json",
    ...
    test_content_matches(
        change_root / "spec.md",
        r"status\.json",
        "Spec documents status.json file",
        "Status Tracking",
    )
    test_content_matches(
        change_root / "spec.md",
        r"status\.json.*schema|schema.*status\.json",
        "Spec includes status.json schema",
        "Status Tracking",
    )
    test_content_matches(
        change_root / "spec.md",
        r"workflow.*resumption|resume.*workflow",
        "Spec documents workflow resumption capability",
        "Status Tracking",
    )
    test_content_matches(
        change_root / "spec.md",
        r"status\.json.*track|observability",
        "Spec documents status.json for state tracking",
        "Status Tracking",
    )
    ...
    test_content_matches(
        change_root / "spec.md",
        r"Pre-Step.*Validation|Validation.*Hooks",
        "Spec documents pre-step validation hooks",
        "Pre-Step Hooks",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"Appendix.*C.*Hooks|Pre-Step.*Validation",
        "Proposal documents pre-step hooks",
        "Pre-Step Hooks",
    )
    test_content_matches(
        change_root / "spec.md",
        r"Stage\s+0.*Python.*environment|environment.*validation",
        "Spec defines Stage 0 environment validation",
        "Pre-Step Hooks",
    )
    test_content_matches(
        change_root / "spec.md",
        r"Stage\s+10.*git.*clean|git.*state",
        "Spec defines Stage 10 git validation",
        "Pre-Step Hooks",
    )
    ...
    test_content_matches(
        change_root / "spec.md",
        r"Conventional.*Commit|commit.*format",
        "Spec documents Conventional Commits validation",
        "Conventional Commits",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"Appendix.*E.*Commits|Conventional.*Commits",
        "Proposal documents Conventional Commits format",
        "Conventional Commits",
    )
    test_content_matches(
        change_root / "spec.md",
        r"interactive.*fixer|--no-verify",
    ...
    test_content_matches(
        change_root / "spec.md",
        r"\[ \]\s+.*--lane.*flag",
        "Acceptance criteria includes --lane flag requirement",
        "Acceptance Criteria",
    )
    test_content_matches(
        change_root / "spec.md",
        r"\[ \]\s+.*parallelization|parallel",
        "Acceptance criteria includes parallelization requirement",
        "Acceptance Criteria",
    )
    test_content_matches(
        change_root / "spec.md",
        r"\[ \]\s+.*quality.*gate",
        "Acceptance criteria includes quality gates requirement",
        "Acceptance Criteria",
    )
    test_content_matches(
        change_root / "spec.md",
        r"\[ \]\s+.*status\.json",
        "Acceptance criteria includes status.json requirement",
        "Acceptance Criteria",
    )
    ...
    test_content_matches(
        change_root / "spec.md",
        r"openspec/changes.*status\.json",
        "Spec documents status.json file path",
        "File Operations",
    )
    test_content_matches(
        change_root / "spec.md",
    ...
    test_content_matches(
        change_root / "proposal.md",
        r"status\.json.*write|emit.*status|write.*status",
    ...
    test_content_matches(
        change_root / "spec.md",
        r"<\s*5.*minute|5.*minute.*<|cycle.*time",
        "Spec documents <5 minute cycle time goal for docs lane",
        "Performance Metrics",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"80%.*adoption|adoption.*80%",
        "Proposal documents 80% adoption target",
        "Performance Metrics",
    )
    test_content_matches(
        change_root / "spec.md",
        r"Success.*Metrics|Measurable.*Success",
        "Spec includes success metrics",
        "Performance Metrics",
    )
    test_content_matches(
        change_root / "proposal.md",
        r"status\.json.*track|track.*status\.json",
    ...
    test_file_exists(
        change_root / "implement.py", "implement.py exists", "Implement Engine"
    )
    test_line_count(
        change_root / "implement.py",
        250,
        "implement.py has implementation logic (250+ lines)",
        "Implement Engine",
    )
    ...
    try:
        ...
        if (
            "ACTIVE IMPLEMENTATION ENGINE" in implement_source
            or "implement_lane_selection" in implement_source
        ):
            _record_pass(
                "Active implementation engine detected",
                "Found phase-based implementation functions",
                "Implement Engine",
            )
        else:
            _record_fail(
                "Active implementation engine detected",
                "Old task registry structure still present",
                "Implement Engine",
            )
        ...
        if "def main():" in implement_source and "if __name__" in implement_source:
            _record_pass(
                ...
                "Main orchestrator found",
                "Implement Engine",
            )
        else:
            _record_fail(
                "main() orchestrator function defined",
                "Main orchestrator not found",
                "Implement Engine",
            )
        ...
        if "results =" in implement_source or '"completed"' in implement_source:
            if '"failed"' in implement_source and '"files_created"' in implement_source:
                _record_pass(
                    ...
                    "Results dict with completed/failed/files tracking",
                    "Implement Engine",
                )
            else:
                _record_fail(
                    "Results tracking structure",
                    "Incomplete results tracking",
                    "Implement Engine",
                )
        else:
            _record_fail(
                "Results tracking structure",
                "Results tracking not found",
                "Implement Engine",
            )
        ...
        if any(ord(c) > 127 for c in implement_source):
            ...
            lines_with_unicode = [
                l for l in implement_source.split("\n") if any(ord(c) > 127 for c in l)
            ]
            problematic = [
                l
                for l in lines_with_unicode
                if not l.strip().startswith("#") and '"""' not in l
            ]
            if problematic:
                _record_fail(
                    "No problematic unicode in code",
                    f"Found unicode in executable code",
                    "Implement Engine",
                )
            else:
                _record_pass(
                    "Unicode characters in code are safe",
                    "Found unicode only in comments/docstrings",
                    "Implement Engine",
                )
        else:
            _record_pass(
        ...
        _record_fail(
            "implement.py analysis",
            f"Error analyzing implement.py: {e}",
            "Implement Engine",
        )
    ...
    try:
        ...
        if "Completed:" in output and "Failed:" in output:
            _record_pass(
                "main() orchestrator executes successfully",
                "Implementation engine ran and produced output",
                "Implement Execution",
            )
        else:
            _record_fail(
                "main() orchestrator executes successfully",
                f"Unexpected output. Return code: {result.returncode}",
                "Implement Execution",
            )
        ...
        if "[OK] Lane selection" in output or "Lane selection flag" in output:
            _record_pass(
                "Phase 1: Lane selection generates scripts/workflow.py modification",
                "Lane selection phase executed",
                "Implement Execution",
            )
        elif "[SKIP]" in output:
            _record_pass(
                "Phase 1: Lane selection detects existing implementation",
                "Lane selection already implemented (skip is OK)",
                "Implement Execution",
            )
        else:
            _record_fail(
                "Phase 1: Lane selection generates modifications",
        ...
        if "[OK] Quality gates module" in output or "Quality gates" in output:
            _record_pass(
                "Phase 2: Quality gates creates scripts/quality_gates.py",
                "Quality gates module created",
                "Implement Execution",
            )
        else:
            _record_fail(
                "Phase 2: Quality gates creates module",
        ...
        if "[OK] Status tracking" in output or "status.json" in output:
            _record_pass(
                "Phase 3: Status template creates status.json",
                "Status tracking template created",
                "Implement Execution",
            )
        else:
            _record_fail(
                "Phase 3: Status template creates file",
                "Phase 3 did not complete successfully",
                "Implement Execution",
            )

        # Overall success metric
        if result.returncode == 0 and (
            "Completed: 3" in output or "Completed: 2" in output
        ):
            _record_pass(
                "All implementation phases completed successfully",
                "Active implementation engine produced expected results",
                "Implement Execution",
            )
        else:
            _record_fail(
                "All implementation phases completed",
                f"Return code: {result.returncode}, output incomplete",
                "Implement Execution",
            )
        ...
        _record_fail(
            "implement.py execution",
            f"Error executing implement.py: {e}",
            "Implement Execution",
        )
    ...
    if workflow_file.exists():
        ...
        if "LANE_MAPPING" in workflow_content:
            _record_pass(
                "Lane selection added to scripts/workflow.py",
                "LANE_MAPPING dictionary found",
                "Generated Artifacts",
            )
        else:
            _record_fail(
                "Lane selection added to workflow.py",
                "LANE_MAPPING not found in file",
                "Generated Artifacts",
            )

        if "--lane" in workflow_content:
            _record_pass(
                "--lane argument added to scripts/workflow.py",
                "--lane flag found in argparse",
                "Generated Artifacts",
            )
        else:
            _record_fail(
                "--lane argument added to workflow.py",
                "--lane flag not found",
                "Generated Artifacts",
            )
    else:
        _record_fail(
            "scripts/workflow.py exists", "File not found", "Generated Artifacts"
        )
    ...
    if quality_gates_file.exists():
        _record_pass(
            "Quality gates module created at scripts/quality_gates.py",
            "File exists and was generated",
            "Generated Artifacts",
        )
        ...
        if "class QualityGates:" in quality_content:
            _record_pass(
                "QualityGates class defined in quality_gates.py",
                "Class definition found",
                "Generated Artifacts",
            )
        else:
            _record_fail(
                "QualityGates class defined", "Class not found", "Generated Artifacts"
            )

        if "def run_all(" in quality_content:
            _record_pass(
                "run_all() method defined in quality_gates.py",
                "Method found",
                "Generated Artifacts",
            )
        else:
            _record_fail(
                "run_all() method defined", "Method not found", "Generated Artifacts"
            )

        if "ruff" in quality_content and "mypy" in quality_content:
            _record_pass(
                "Quality tools (ruff, mypy, pytest, bandit) configured",
                "All tools found in module",
                "Generated Artifacts",
            )
        else:
            _record_fail(
                "Quality tools configured", "Not all tools found", "Generated Artifacts"
            )
    else:
        _record_fail(
            "Quality gates module exists at scripts/quality_gates.py",
            "File not found",
            "Generated Artifacts",
        )
    ...
    if status_file.exists():
        _record_pass(
            "Status tracking template created at status.json",
            "File exists and was generated",
            "Generated Artifacts",
        )
        ...
        try:
            ...
            if not missing:
                _record_pass(
                    "status.json has required schema",
            ...
            _record_fail(
                "status.json is valid JSON", "JSON parse error", "Generated Artifacts"
            )
    else:
        _record_fail(
            "Status tracking template exists at status.json",
            "File not found",
            "Generated Artifacts",
        )
    ...
    for category, stats in sorted(test_results["categories"].items()):
        ...
        print(
            f"{category:.<40} {passed:>3} PASS, {failed:>3} FAIL, {skipped:>3} SKIP ({pct:>5.1f}%)"
        )

...
Precedence: environment variables > agent/config.yaml > code defaults.
...
def get_settings() -> Settings:
    ...
    # Compute agent_url from api_port if not explicitly provided
    if not data.get("agent_url"):
        port = data.get("api_port", Settings().api_port)
        data["agent_url"] = f"http://127.0.0.1:{port}"

...
app = FastAPI(
    title=(
        "Obsidian AI Agent - Enterprise Edition"
        if ENTERPRISE_AVAILABLE
        else "Obsidian AI Agent"
    ),
    lifespan=lifespan,
)
...
try:
    ...
    log_config = {
        ...
        "log_dir": os.environ.get("LOG_DIR", "./agent/logs"),
...
def init_services():
    ...
    try:
        ...
        cache_dir = os.getenv("CACHE_DIR", "./agent/cache")
...
def _settings_to_dict(s: object) -> dict:
    ...
    try:
        from agent.settings import _ALLOWED_UPDATE_KEYS
...
async def get_config():
    ...
    try:
        with error_context("get_configuration", reraise=False):
            ...
            # Back-compat: tests expect a field named 'agent/vector_db' indicating the default path
            try:
                from pathlib import Path

                project_root = Path(get_settings().project_root)
                filtered["agent/vector_db"] = str(
...
PID_FILE = pathlib.Path("agent_server.pid")
DEFAULT_PORT = 8000
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Obsidian AI Agent Backend CLI")
    parser.add_argument(
...
# Ensure this module can be accessed as 'agent.agent' regardless of import mode
try:
    ...
    if this_mod is not None:
        # Expose attribute for patch targets: agent.agent -> this module
        this_mod.__dict__.setdefault("backend", this_mod)
        # Mark as package-like: provide __path__ so import system allows 'agent.agent'
    ...
    logging.error(f"Exception in agent.agent (module access): {e}")

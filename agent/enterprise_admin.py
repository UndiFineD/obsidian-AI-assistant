...
class EnterpriseAdminDashboard:
    ...
    async def get_dashboard_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to load dashboard"
            ) from None
    ...
    async def _get_tenant_metrics(self) -> Dict[str, Any]:
        ...
        total_users = sum(
            t.get("user_count", 0) for t in self.tenant_manager.usage_metrics.values()
        )
        ...
        return {
            ...
            "total_revenue_monthly": sum(
                t.billing_config.get("monthly_cost", 0) for t in tenants
            ),
    ...
    async def _get_compliance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_subject_requests_30d": len(
                [
                    req
                    for req in self.gdpr_manager.data_subject_requests.values()
    ...
    async def _get_active_alerts(self) -> List[Dict[str, Any]]:
        ...
        active_alerts = [
            alert for alert in self.system_alerts.values() if alert.resolved_at is None
        ]
        ...
        try:
            ...
            return {
                ...
                "filters": {
                    ...
                    "available_tenants": [
                        t.tenant_id for t in self.tenant_manager.list_tenants()
                    ],
                },
            }
            ...
            raise HTTPException(
                status_code=500, detail="Failed to load user management"
            ) from None
    ...
    async def get_tenant_management(self) -> Dict[str, Any]:
        ...
        try:
                ...
                tenant_data.append(
                    {
                        ...
                        "limits": {
                            ...
                            "max_api_calls_monthly": tenant.limits.get(
            ...
            return {
                ...
                "summary": {
                    ...
                    "total_monthly_revenue": sum(
                        t.billing_info.get("monthly_cost", 0) for t in tenants
                    ),
                },
            }
            ...
            raise HTTPException(
                status_code=500, detail="Failed to load tenant management"
            ) from None
    ...
    async def get_security_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "access_controls": {
                    "recent_reviews": [
                        ...
                        for review in list(self.soc2_manager.access_reviews.values())[
                            -10:
                        ]
                    ],
                    "overdue_reviews": len(
                        [
                            r
                            for r in self.soc2_manager.access_reviews.values()
                            if r.next_review_date
                            and r.next_review_date < datetime.utcnow()
                        ]
                    ),
                },
                "vulnerabilities": {
                    "recent_assessments": [
                        ...
                        for va in list(
                            self.soc2_manager.vulnerability_assessments.values()
                        )[-5:]
                    ]
                },
                "compliance": {
                    "soc2_score": soc2_report["overall_compliance_score"],
                    "control_effectiveness": soc2_report["control_testing"][
                        "effectiveness_rate"
                    ],
                },
            }
            ...
            raise HTTPException(
                status_code=500, detail="Failed to load security dashboard"
            ) from None
    ...
    async def get_compliance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
                ...
                "gdpr": {
                    ...
                    "consent_metrics": {
                        ...
                        "active_consents": len(
                            [
                                c
                                for c in self.gdpr_manager.consent_records.values()
            ...
            raise HTTPException(
                status_code=500, detail="Failed to load compliance dashboard"
            ) from None

    async def create_system_alert(
        self, severity: str, title: str, description: str, component: str
    ) -> SystemAlert:
    ...
    async def bulk_user_action(
        self, action: str, user_ids: List[str], parameters: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        for user_id in user_ids:
            try:
                if action == "deactivate":
                    ...
                    success_count += 1
                    results.append(
                        {"user_id": user_id, "status": "password_reset_sent"}
                    )
                    ...
                    if role and self.rbac_manager.is_valid_role(role):
                        ...
                        success_count += 1
                        results.append(
                            {"user_id": user_id, "status": f"role_{role}_assigned"}
                        )
                    else:
                        failed_count += 1
    ...
    def _register_endpoints(self):
        ...
        async def get_security_dashboard(request: Request):
            ...
            if "admin" not in user_roles and "security_admin" not in user_roles:
                raise HTTPException(
                    status_code=403, detail="Security admin access required"
                )
        ...
        async def get_compliance_dashboard(request: Request):
            ...
            if "admin" not in user_roles and "compliance_admin" not in user_roles:
                raise HTTPException(
                    status_code=403, detail="Compliance admin access required"
                )
        ...
        async def bulk_user_action(action_data: Dict[str, Any], request: Request):
            ...
            if not action or not user_ids:
                raise HTTPException(
                    status_code=400, detail="Action and user_ids required"
                )

            return await self.admin_dashboard.bulk_user_action(
                action, user_ids, parameters
            )

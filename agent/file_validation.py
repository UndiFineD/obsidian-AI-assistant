...
class FileValidator:
    ...
    def __init__(self, allowed_types: Optional[List[str]] = None):
        ...
        if MAGIC_AVAILABLE:
            try:
                ...
                print(
                    "[FileValidator] Warning: python-magic available but failed to "
                    "initialize"
                )
                self.magic_available = False
        else:
            print(
                "[FileValidator] Info: python-magic not available, using basic "
                "validation"
            )
            self.magic_available = False
    ...
    def _detect_by_magic_bytes(self, file_content: bytes) -> Optional[Tuple[str, str]]:
        """Detect using leading magic bytes."""
    ...
    def _detect_by_python_magic(self, file_content: bytes) -> Optional[Tuple[str, str]]:
        """Detect using python-magic, if available."""
    ...
    def _detect_by_extension(self, filename: str) -> Optional[Tuple[str, str]]:
    ...
    def _detect_by_mimetypes(self, filename: str) -> Optional[Tuple[str, str]]:
        """Detect using Python mimetypes as last resort."""
    ...
    def detect_file_type(self, file_content: bytes, filename: str) -> Tuple[str, str]:
        ...
        """
        detected = self._detect_by_magic_bytes(file_content)
        if detected:
            return detected

        detected = self._detect_by_python_magic(file_content)
        if detected:
            return detected

        detected = self._detect_by_extension(filename)
        if detected:
            return detected

        detected = self._detect_by_mimetypes(filename)
        if detected:
            return detected

        return self._raise_unsupported(filename)

    def _raise_unsupported(self, filename: str) -> Tuple[str, str]:
        """Helper to raise a consistent error when detection fails."""
    ...
    def validate_file_size(self, file_content: bytes, file_type: str) -> None:
        ...
        if size_mb > max_size:
            raise FileValidationError(
                f"File too large: {size_mb:.1f}MB (max {max_size}MB for {file_type} "
                "files)"
            )
        ...
        if filename.lower().endswith(".pdf"):
            ...
            for dangerous in dangerous_pdf_content:
                if dangerous in file_content:
                    raise FileValidationError(
                        "PDF contains potentially dangerous content"
                    )

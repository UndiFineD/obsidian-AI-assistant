...
class AIRightPaneView {
    ...
    async open() {
        ...
        // Cancel button
        const cancelBtn = this.container.createEl('button', {
            text: 'Cancel',
            cls: 'ai-cancel-btn',
        });
    ...
    async renderBackendStatus(div) {
        ...
        const reloadBtn = div.createEl('button', {
            text: offline ? 'Check again' : 'Reload config',
        });
    ...
    renderLogin(div) {
        ...
        if (this.isLoggedIn) {
            div.createEl('span', { text: `Logged in as: ${this.currentUser}` });
            ...
            div.createEl('label', { text: 'User Login:' });
            const userInput = div.createEl('input', {
                type: 'text',
                placeholder: 'Username',
                cls: 'ai-login-input',
            });
            const passInput = div.createEl('input', {
                type: 'password',
    ...
    async authenticate(username, password) {
        try {
            const data = await this.backendClient.post('/api/auth/login', { username, password });
    ...
    async logout() {
        try {
            if (this.authToken) {
                await this.backendClient.post('/api/auth/logout', {});
            }
        } catch (error) {
    ...
    renderConfig(div) {
        ...
        const serverInput = div.createEl('input', {
            type: 'text',
            value: this.plugin.settings.backendUrl,
        });
        serverInput.onchange = (e) => {
            this.plugin.settings.backendUrl = e.target.value;
    ...
    renderInput(div) {
        ...
        const textInput = div.createEl('textarea', {
        ...
        const voiceBtn = buttonContainer.createEl('button', {
            text: this.isVoiceRecording ? 'Stop Recording' : 'Voice',
    ...
    async askQuestion(question) {
        ...
        try {
            const data = await this.backendClient.post(
                '/ask',
                { question: question, prefer_fast: true, include_sources: true },
                this.cancelController.signal
            );

            // Add to analytics
            this.analyticsState.qaHistory.push({
            ...
            this.analyticsState.modelUsage[model] =
                (this.analyticsState.modelUsage[model] || 0) + 1;
            ...
            if (error.name === 'AbortError') {
                new Notice('Question cancelled.');
            } else {
                new Notice('Error asking question: ' + error.message);
                throw error;
            }
        } finally {
            this.cancelController = null;
    ...
    async stopVoiceRecording() {
        try {
            ...
            if (audioBlob.size > 0) {
                new Notice('🎙️ Processing voice input...');

                const transcription = await this.voiceRecorder.sendToBackend(
                    audioBlob,
    ...
    cancelCurrentTask() {
        ...
        if (cancelled) {
            new Notice('Current operations cancelled.');
        } else {
            new Notice('No active operations to cancel.');
        }
    }
    ...
    renderAnalytics(div) {
        ...
        statsDiv.createEl('p', {
            text: `📄 Processed notes: ${Object.keys(this.analyticsState.processedNotes).length}`,
        });
        statsDiv.createEl('p', {
            text: `💬 QA interactions: ${this.analyticsState.qaHistory.length}`,
        });
        statsDiv.createEl('p', {
            text: `🤖 Models used: ${Object.keys(this.analyticsState.modelUsage).length}`,
        });

        if (this.analyticsState.lastUpdated) {
            statsDiv.createEl('p', {
                text: `🔄 Last updated: ${this.analyticsState.lastUpdated.toLocaleTimeString()}`,
                cls: 'analytics-timestamp',
            });
        }

        // Model usage breakdown
        if (Object.keys(this.analyticsState.modelUsage).length > 0) {
            div.createEl('h4', { text: 'Model Usage' });
        ...
        // Recent QA history
        if (this.analyticsState.qaHistory.length > 0) {
            div.createEl('h4', { text: 'Recent Questions' });
            const historyDiv = div.createEl('div', { cls: 'qa-history' });

            const recentQuestions = this.analyticsState.qaHistory.slice(-5).reverse();
            recentQuestions.forEach((entry) => {
                const entryDiv = historyDiv.createEl('div', { cls: 'qa-entry' });
                const timestamp = new Date(entry.timestamp).toLocaleTimeString();
                entryDiv.createEl('div', {
                    text: `[${timestamp}] ${entry.prompt.substring(0, 50)}${entry.prompt.length > 50 ? '...' : ''}`,
                    cls: 'qa-question',
                });
                if (entry.response_time) {
                    entryDiv.createEl('div', {
                        text: `⚡ ${entry.response_time}ms | Model: ${entry.model}`,
                        cls: 'qa-meta',
                    });
                }
            });
    ...
    stopAnalyticsPolling() {
        if (this.analyticsPollingId) {
            this.backendClient.stopPolling(this.analyticsPollingId);
            this.analyticsPollingId = null;
        }
    }
    ...
    renderTaskQueue(div) {
        ...
        statusDiv.createEl('p', {
            text: `Status: ${isRunning ? '🟢 Running' : '🔴 Stopped'} | Tasks: ${queueLength}`,
        ...
        if (tasks.length === 0) {
            div.createEl('p', { text: 'No tasks in queue.', cls: 'empty-queue' });
            return;
        }

        const taskList = div.createEl('div', { cls: 'task-list' });
        tasks.slice(0, 10).forEach((task, index) => {
            const taskEl = taskList.createEl('div', {
                cls: `ai-task-item ${task.status || 'pending'}`,
            });
            ...
            if (task.priority && task.priority > 1) {
                taskHeader.createEl('span', { text: `⭐${task.priority} `, cls: 'task-priority' });
            }

            const removeBtn = taskHeader.createEl('button', { text: '✕', cls: 'task-remove-btn' });
            removeBtn.onclick = () => {
                this.taskQueue.queue.splice(index, 1);
            ...
            if (task.timestamp) {
                taskEl.createEl('div', {
                    text: `⏰ ${new Date(task.timestamp).toLocaleTimeString()}`,
                    cls: 'task-timestamp',
                });
            }
        });
    ...
    destroy() {
        ...
        if (this.isVoiceRecording) {
            this.stopVoiceRecording().catch(console.error);
        }

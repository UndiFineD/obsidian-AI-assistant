...
class EnterpriseAuth {
    ...
    async initialize() {
        try {
            ...
            console.error('Failed to initialize enterprise auth:', error);
            await this.clearAuth();
        }
    }
    ...
    async loadStoredAuth() {
        try {
            const storedAuth = localStorage.getItem('enterprise_auth');
            if (storedAuth) {
    ...
    async validateToken() {
        try {
            ...
            const response = await this.backendClient.request('/sso/validate', {
                method: 'POST',
    ...
    async refreshAuthToken() {
        try {
            ...
            const response = await this.backendClient.request('/sso/refresh', {
                method: 'POST',
    ...
    async getAvailableProviders() {
        try {
            const response = await this.backendClient.request('/sso/providers', { method: 'GET' });
    ...
    async initiateLogin(provider) {
        try {
            const response = await this.backendClient.request('/sso/login', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                throw new Error(response.message || 'Login initiation failed');
            }
        } catch (error) {
    ...
    async checkAuthCallback() {
        try {
            ...
            if (code && state) {
                ...
                const response = await this.backendClient.request('/sso/callback', {
                    method: 'POST',
            ...
            if (tempAuth) {
                localStorage.removeItem('temp_enterprise_auth');
            ...
            console.error('Auth callback check failed:', error);
            throw error;
        }
    }
    ...
    async logout() {
        try {
            if (this.authToken) {
                ...
                await this.backendClient.request('/sso/logout', {
                    method: 'POST',
                    data: { token: this.authToken },
                });
            }
    ...
    createLoginInterface(containerEl) {
        containerEl.empty();
        ...
        headerEl.createEl('p', {
            text: "Sign in with your organization's SSO provider",
            cls: 'login-subtitle',
        });
        ...
        const directLoginBtn = altLoginEl.createEl('button', {
            text: 'Direct Login',
    ...
    async createProviderSelection(containerEl) {
        try {
            ...
            providers.forEach((provider) => {
                const providerBtn = providerEl.createEl('button', {
                    cls: `provider-btn ${provider.name.toLowerCase()}`,
                });

                // Provider icon and name
                const iconEl = providerBtn.createEl('span', {
                    cls: `provider-icon ${provider.name.toLowerCase()}`,
                });
                iconEl.textContent = this.getProviderIcon(provider.name);

                providerBtn.createEl('span', {
                    text: `Sign in with ${provider.display_name}`,
                    cls: 'provider-text',
                });
                ...
                providerBtn.addEventListener('click', async () => {
                    try {
                        ...
                        this.plugin.onAuthenticationChanged(
                            true,
                            this.currentUser,
                            this.currentTenant
                        );
            ...
            containerEl.createEl('p', {
                text: 'Failed to load SSO providers',
                cls: 'error-message',
            });
        }
    }

    showDirectLogin(containerEl) {
        const directLoginEl = containerEl.createEl('div', { cls: 'direct-login' });

        directLoginEl.createEl('h3', { text: 'Direct Login' });

        // Email input
        const emailInput = directLoginEl.createEl('input', {
            type: 'email',
        ...
        // Password input
        const passwordInput = directLoginEl.createEl('input', {
            type: 'password',
        ...
        // Tenant input
        const tenantInput = directLoginEl.createEl('input', {
            type: 'text',
        ...
        // Login button
        const loginBtn = directLoginEl.createEl('button', {
            text: 'Sign In',
        ...
        loginBtn.addEventListener('click', async () => {
            try {
                ...
                const response = await this.backendClient.request('/sso/login/direct', {
                    method: 'POST',
                    data: {
    ...
    createUserProfile(containerEl) {
        if (!this.isAuthenticated()) {
            containerEl.createEl('p', { text: 'Not authenticated' });
            return;
        }
        ...
        if (this.currentUser.roles && this.currentUser.roles.length > 0) {
            const rolesEl = detailsEl.createEl('div', { cls: 'user-roles' });
            this.currentUser.roles.forEach((role) => {
                rolesEl.createEl('span', { text: role, cls: 'role-badge' });
            });
        }
        ...
        const logoutBtn = actionsEl.createEl('button', { text: 'Sign Out', cls: 'btn btn-danger' });
    ...
    getProviderIcon(providerName) {
        ...
        return icons[providerName.toLowerCase()] || 'f511';
    }

    showError(containerEl, message) {
        let errorEl = containerEl.querySelector('.auth-error');
        if (!errorEl) {
            errorEl = containerEl.createEl('div', { cls: 'auth-error' });
        }
    ...
    isAuthenticated() {
        return !!(this.authToken && this.currentUser && this.currentTenant);
    }

...
class AuditLogger:
    ...
    def log_access_event(self, client_id: str, endpoint: str, method: str, 
                        status: int, user_agent: str = "", details: Dict = None):
    ...
    def get_recent_events(self, severity: Optional[ThreatLevel] = None, 
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        high_severity_events = [e for e in recent_events 
        ...
        return {
            ...
            "recent_threat_types": list(set(e.event_type for e in high_severity_events)),
...
class AdvancedInputValidator:
    ...
    def sanitize_input(self, input_str: str, max_length: int = 10000, 
    ...
    def validate_json_input(self, json_str: str, max_depth: int = 10, 
                           max_keys: int = 100) -> tuple[bool, Optional[Dict]]:
    ...
    def _check_json_complexity(self, obj: Any, max_depth: int, max_keys: int, 
        ...
        if isinstance(obj, dict):
            if len(obj) > max_keys:
                return False
            return all(self._check_json_complexity(v, max_depth, max_keys, current_depth + 1) 
                      for v in obj.values())
        
        elif isinstance(obj, list):
            if len(obj) > max_keys:
                return False
            return all(self._check_json_complexity(item, max_depth, max_keys, current_depth + 1) 
                      for item in obj)
...
class ComplianceManager:
    ...
    def __init__(self, audit_logger: AuditLogger):
        ...
        self.data_retention_days = int(os.getenv("DATA_RETENTION_DAYS", "2555"))  # 7 years
    ...
    def log_data_access(self, user_id: str, data_type: str, action: str, 
                       purpose: str, legal_basis: str = "legitimate_interest"):
...
class SecurityConfig:
    ...
    def __init__(self):
        ...
        self.enable_threat_detection = os.getenv("ENABLE_THREAT_DETECTION", "true").lower() == "true"
        ...
        self.input_validator = AdvancedInputValidator() if self.enable_threat_detection else None
        self.compliance_manager = ComplianceManager(self.audit_logger) if self.audit_logger else None
    ...
    def validate_and_sanitize_input(self, input_data: str, source: str = "unknown") -> tuple[bool, str]:
    ...
    def log_access_event(self, client_id: str, endpoint: str, method: str, 
        ...
        """Log access event"""
        if self.audit_logger:
            self.audit_logger.log_access_event(client_id, endpoint, method, status, user_agent, details)
...
def sanitize_input(input_str: str, max_length: int = 10000) -> str:
    ...
    if advanced_security_config.input_validator:
        return advanced_security_config.input_validator.sanitize_input(input_str, max_length)
...
def log_security_event(event_type: str, severity: ThreatLevel, 
                      source: str, description: str, details: Dict = None):
    """Log security event using global config"""
    advanced_security_config.log_security_event(event_type, severity, source, description, details)
...
def log_data_access(user_id: str, data_type: str, action: str, purpose: str):
    ...
    if advanced_security_config.compliance_manager:
        advanced_security_config.compliance_manager.log_data_access(user_id, data_type, action, purpose)
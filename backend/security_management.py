"""
Security Management API for Obsidian AI Assistant
...
from .security_hardening import (
    SecurityLevel, AuthenticationMethod, get_security_status,
    SessionManager, APIKeyManager, ThreatDetector
)
...
class SecurityConfigUpdate(BaseModel):
    ...
    security_level: Optional[str] = Field(None, pattern="^(minimal|standard|enhanced|maximum)$")
...
async def get_security_system_status():
    ...
    with error_context("security_status", reraise=False):
        ...
        session_stats = {
            ...
            "total_users": len(set(s["user_id"] for s in session_manager.active_sessions.values()))
        }
        
        api_key_stats = {
            "active_keys": len([k for k in api_key_manager.active_keys.values() if k["is_active"]]),
            "revoked_keys": len(api_key_manager.revoked_keys),
            "total_usage_24h": sum(
                len([u for u in usage if u["timestamp"] > datetime.utcnow() - timedelta(hours=24)])
        ...
        threat_stats = {
            ...
            "high_threat_ips": len([
                ip for ip, behavior in threat_detector.behavioral_tracker.items()
...
async def list_active_sessions(
    ...
    with error_context("list_sessions", reraise=False):
        security_logger.info("Sessions list requested", extra={
...
async def perform_session_action(request: SessionActionRequest):
    ...
    with error_context("session_action", reraise=False):
        ...
        if request.action == "terminate":
            session_manager.terminate_session(request.session_id, request.reason or "admin_action")
        elif request.action == "block":
            session_manager.block_session(request.session_id, request.reason or "security_violation")
        ...
        security_logger.info("Session action performed", extra={
                ...
                for historical_session in user_history:
                    if historical_session.get("session_id") == session_id:
                        return SessionInfo(
                            ...
                            last_activity=historical_session.get("terminated_at", historical_session["last_activity"]).isoformat(),
...
async def create_api_key(request: CreateAPIKeyRequest):
    ...
    with error_context("create_api_key", reraise=False):
        ...
        security_logger.info("API key created", extra={
            "key_id": key_data["key_id"],
            "name": request.name,
            "permissions": request.permissions
        })
...
async def revoke_api_key(key_id: str, reason: str = Query("admin_revocation")):
    ...
    with error_context("revoke_api_key", reraise=False):
        ...
        security_logger.warning("API key revoked", extra={
            "key_id": key_id,
            "reason": reason
        })
...
async def list_threat_events(
    ...
    with error_context("list_threats", reraise=False):
        security_logger.info("Threat events requested", extra={
            "hours": hours,
            "min_score": min_score,
            "limit": limit
        })
...
@router.post("/config")
async def update_security_config(request: SecurityConfigUpdate, background_tasks: BackgroundTasks):
    ...
    with error_context("update_security_config", reraise=False):
        ...
        if request.session_timeout_hours:
            ...
            session_manager.session_timeout = timedelta(hours=request.session_timeout_hours)

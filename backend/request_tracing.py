...
from .logging_framework import get_logger, LogCategory
...
class RequestTracer:
    ...
    def _update_endpoint_stats(self, request_record: Dict[str, Any]):
        ...
        stats["min_duration_ms"] = min(stats["min_duration_ms"], request_record["duration_ms"])
        stats["max_duration_ms"] = max(stats["max_duration_ms"], request_record["duration_ms"])
    ...
    def get_endpoint_stats(self, endpoint: Optional[str] = None) -> Dict[str, Any]:
        ...
        for ep, stats in self.endpoint_stats.items():
            ...
            if ep_stats["total_requests"] > 0:
                ep_stats["avg_duration_ms"] = ep_stats["total_duration_ms"] / ep_stats["total_requests"]
                ep_stats["error_rate"] = ep_stats["error_count"] / ep_stats["total_requests"]
                ep_stats["slow_rate"] = ep_stats["slow_count"] / ep_stats["total_requests"]
            result[ep] = ep_stats
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        slow_count = sum(1 for r in self.request_history if r["duration_ms"] > self.slow_threshold)
...
class RequestTracingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            response.headers["X-Response-Time"] = f"{(time.perf_counter() - context['start_time']) * 1000:.2f}ms"
...
@asynccontextmanager
async def trace_operation(operation_name: str, context: Optional[Dict[str, Any]] = None):
    """

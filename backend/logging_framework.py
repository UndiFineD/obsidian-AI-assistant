    ...
    def redact_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                redacted[key] = [self.redact_pii(str(item)) if isinstance(item, str) else item for item in value]
            else:
                redacted[key] = value
...
class RequestTracker:
    ...
    def get_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'elapsed_time': time.time() - getattr(self._local, 'start_time', time.time())
        }
...
class LogManager:
    ...
    def _setup_file_handlers(self):
        ...
        app_handler.setFormatter(StructuredFormatter(include_pii=self.config['include_pii']))
        ...
        error_handler.setFormatter(StructuredFormatter(include_pii=self.config['include_pii']))
    ...
    def _setup_audit_handler(self):
        ...
        audit_handler.setFormatter(StructuredFormatter(include_pii=False))  # Never include PII in audit logs
    ...
    def get_logger(self, name: str, category: LogCategory = LogCategory.SYSTEM) -> logging.Logger:
        ...
        if not hasattr(logger, '_category_filter_added'):
            logger.addFilter(lambda record: setattr(record, 'category', category.value) or True)
            logger._category_filter_added = True
        
        return logger
    
    def log_audit_event(self, event_type: str, user_id: str = None, resource: str = None, 
                       action: str = None, outcome: str = None, **kwargs):
        ...
        self._loggers['audit'].log(LogLevel.AUDIT.value, 'Audit event', extra=audit_data)
    
    def log_security_event(self, event_type: str, severity: str = 'medium', 
                          threat_level: str = 'low', **kwargs):
        """Log security event"""
        ...
        self._loggers['security'].log(level, f'Security event: {event_type}', extra=security_data)
    ...
    def log_performance_metric(self, operation: str, duration: float, **kwargs):
        ...
        self._loggers['performance'].info(f'Performance: {operation}', extra=performance_data)
        ...
        if log_dir.exists():
            ...
            stats['log_files'] = {
                ...
                'files': [
                    ...
                    for f in log_files if f.is_file()
                ]
            }
...
def _auto_initialize():
    ...
    if 'LOG_INCLUDE_PII' in os.environ:
        config['include_pii'] = os.environ['LOG_INCLUDE_PII'].lower() in ('true', '1', 'yes')

...
class HealthMonitor:
    ...
    def _initialize_thresholds(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            ...
            "memory": {"warning": 75.0, "error": 90.0, "critical": 95.0},
            "disk": {"warning": 80.0, "error": 90.0, "critical": 95.0},
            "response_time": {
                "warning": 1000.0,  # ms
                "error": 3000.0,
                "critical": 5000.0,
    ...
    async def check_service_health(
        self, service_name: str, check_function: callable, timeout_seconds: float = 5.0
    ) -> HealthCheckResult:
        """
        ...
        try:
            ...
            await asyncio.wait_for(check_function(), timeout=timeout_seconds)
            ...
            result = HealthCheckResult(
                ...
                consecutive_failures=self.services.get(
                    service_name,
                    HealthCheckResult(
                        service_name=service_name,
    ...
    async def _check_metrics_alerts(self, metrics: SystemMetrics):
        ...
        await self._check_threshold_alert(
            "cpu_usage", "CPU", metrics.cpu_percent, self.alert_thresholds["cpu"]
        )
        ...
        return {
            ...
            "services": {
                name: {
                    ...
                    "last_check": (
                        service.timestamp.isoformat() if service.timestamp else None
                    ),
    ...
    def get_metrics_summary(self, window_minutes: int = 5) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_avg": sum(m.memory_percent for m in recent_metrics)
            / len(recent_metrics),
            "memory_max": max(m.memory_percent for m in recent_metrics),
            "disk_avg": sum(m.disk_usage_percent for m in recent_metrics)
            / len(recent_metrics),
            "connections_avg": sum(m.network_connections for m in recent_metrics)
            / len(recent_metrics),
        }

"""
Advanced Security Hardening System for Obsidian AI Assistant
...
from .advanced_security import SecurityEvent, ThreatLevel, log_security_event
from .error_handling import error_context, SecurityError, ValidationError
from .logging_framework import get_logger, log_audit, log_security, LogCategory
...
class SessionManager:
    ...
    def create_session(self, user_id: str, client_ip: str, user_agent: str) -> str:
        ...
        with error_context("create_session", reraise=False):
            ...
            self.security_logger.info("Session created", extra={
                "session_id": session_id,
                "user_id": user_id,
                "client_ip": client_ip,
    ...
    def validate_session(self, session_id: str, client_ip: str, user_agent: str) -> Optional[Dict[str, Any]]:
    ...
    def terminate_session(self, session_id: str, reason: str):
        ...
        if session:
            ...
            self.session_history[session["user_id"]] = self.session_history[session["user_id"]][-10:]
            
            self.security_logger.info("Session terminated", extra={
                "session_id": session_id,
                "user_id": session["user_id"],
                "reason": reason,
    ...
    def cleanup_expired_sessions(self):
        ...
        self.blocked_sessions = {
            s for s in self.blocked_sessions 
...
class APIKeyManager:
    ...
    def validate_api_key(self, api_key: str, client_ip: str, required_permission: str = "user") -> Optional[Dict[str, Any]]:
...
class RequestSigner:
    ...
    def sign_request(self, method: str, path: str, body: str, timestamp: str, key_id: str = "master") -> str:
    ...
    def validate_request_signature(self, request: Request, signature: str, timestamp: str, key_id: str = "master") -> bool:
        ...
        try:
            ...
            self.security_logger.error("Signature validation error", extra={
                "error": str(e),
...
class ThreatDetector:
    ...
    def __init__(self):
        self.threat_patterns = {
            ...
            "command_injection": [
                r";\s*(rm|del|format)", r";\s*(cat|type)\s+",
                r";\s*(wget|curl)\s+", r";\s*(nc|netcat)\s+"
            ]
        }
    ...
    def analyze_request(self, context: SecurityContext, request_body: str = "") -> float:
    ...
    def _analyze_behavior(self, context: SecurityContext) -> float:
        ...
        if len(behavior["request_times"]) >= 10:
            time_window = (behavior["request_times"][-1] - behavior["request_times"][-10]).total_seconds()
            if time_window > 0:
...
class SecurityHardeningMiddleware(BaseHTTPMiddleware):
    ...
    async def _session_cleanup_task(self):
        ...
        while True:
            try:
                ...
                self.security_logger.error("Session cleanup error", extra={"error": str(e)})
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (self.security_level == SecurityLevel.MINIMAL and 
            context.request_path in self.public_endpoints):
        ...
        try:
            ...
            self.security_logger.debug("Request processed", extra={
                "request_id": context.request_id,
                "client_ip": context.client_ip,
            ...
            self.security_logger.error("Security middleware error", extra={
                "request_id": context.request_id,
                "error": str(e)
            })
            
            # Continue processing for non-security errors
            return await call_next(request)
    
    async def _validate_authentication(self, request: Request, context: SecurityContext) -> Dict[str, Any]:
        ...
        if signature and timestamp:
            if self.request_signer.validate_request_signature(request, signature, timestamp):
                context.auth_method = AuthenticationMethod.SIGNED_REQUEST
                return {"valid": True, "method": "signed_request"}
            else:
                return {"valid": False, "method": "signed_request", "error": "Invalid signature"}
        
        # No valid authentication found
        return {"valid": False, "method": None, "error": "No valid authentication provided"}
...
def create_security_hardening_middleware(security_level: SecurityLevel = SecurityLevel.STANDARD) -> SecurityHardeningMiddleware:

...
def run_coverage_analysis() -> Tuple[float, Dict[str, Dict[str, any]]]:
    ...
    try:
        ...
        result = subprocess.run(
            [python_exe, '-m', 'pytest', 'tests/backend/', '--cov=backend', 
             '--cov-report=xml', '--cov-report=json', '-q'],
            cwd=os.getcwd(),
            capture_output=True,
            text=True,
            timeout=300
...
def categorize_files_by_coverage(coverage_data: Dict[str, Dict]) -> Dict[str, List[str]]:
...
def generate_improvement_plan(opportunities: List[Dict]) -> str:
    ...
    for i, opportunity in enumerate(opportunities[:5], 1):
        ...
        plan.append(f"- **Sample Missing Lines**: {', '.join(map(str, missing_lines[:5]))}")
    ...
    plan.append("python -m pytest tests/backend/test_specific.py --cov=backend/specific.py --cov-report=term-missing")
    plan.append("```")
    plan.append("")
...
def create_coverage_report_html(overall_coverage: float, categories: Dict[str, List[str]], 
...
def main():
    ...
    print(f"🎯 **Target**: 85%")
    print(f"📈 **Status**: {'✅ MEETS TARGET' if overall_coverage >= 85 else '⚠️ BELOW TARGET'}")
    print()
    ...
    if opportunities:
        ...
        for i, opp in enumerate(opportunities[:3], 1):
            print(f"  {i}. {opp['filename']} ({opp['current_coverage']:.1f}% - need {opp['additional_lines_needed']} lines)")
        print()
    ...
    html_dashboard = create_coverage_report_html(overall_coverage, categories, opportunities)
    ...
    print("  4. Run tests locally with: python -m pytest --cov=backend --cov-report=html")

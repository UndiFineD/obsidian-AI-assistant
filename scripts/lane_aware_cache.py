...
class LaneAwareCacheStrategy:
    ...
    def get_eviction_priority(lane: LaneType, entry: CacheEntry) -> float:
        ...
        access_recency = (datetime.now() - datetime.fromisoformat(entry.accessed_at)).total_seconds()
...
class StateSnapshotCache:
    ...
    def cache_checkpoint(self, checkpoint_id: str, state_data: Dict[str, Any], lane: str) -> bool:
        ...
        try:
            with self.lock:
                ...
                if checkpoint_id in self.metadata:
                    self.metadata[checkpoint_id]["accessed_at"] = datetime.now().isoformat()
...
class CacheManager:
    ...
    def get_or_set(
        ...
        if value is not None:
            ...
            entry = CacheEntry(
                ...
                created_at=datetime.now().isoformat(),
                accessed_at=datetime.now().isoformat(),
                expires_at=(datetime.now() + timedelta(seconds=ttl or self.policy.ttl_seconds)).isoformat(),
                tags=tags or [],
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "l3": {"entries": len(self.l3.cache), "hits": self.l3.hits, "misses": self.l3.misses} if self.policy.persistence_enabled else None,
    ...
    def get_checkpoint_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_size_bytes": sum(m.get("size_bytes", 0) for m in self.snapshots.metadata.values()),
...
def measure_cache_performance(cache: CacheManager, iterations: int = 1000) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lane", choices=["docs", "standard", "heavy"], default="standard")
    parser.add_argument("--action", choices=["clear", "metrics", "status", "cleanup"], default="metrics")

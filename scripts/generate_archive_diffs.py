...
def main():
    ...
    print(
        f"{'Status':<12} {'Archive File':<30} {'Current File':<30} {'Diff Lines':<12}"
    )
    print("-" * 84)
    ...
    for archive_name, current_name in comparisons:
        ...
        if not archive_file.exists():
            print(
                f"{'⚠️  MISSING':<12} {archive_name:<30} {current_name:<30} {'N/A':<12}"
            )
            continue

        if not current_file.exists():
            print(
                f"{'❌ NO CURR':<12} {archive_name:<30} {current_name:<30} {'N/A':<12}"
            )
            continue

...
class WorkflowVisualizer:
    ...
    def get_step_info(self, step_num: int, state: WorkflowState) -> StepInfo:
        ...
        if has_checkpoint and self.checkpoint_manager:
            try:
                checkpoint_list = self.checkpoint_manager.list_checkpoints()
                step_checkpoints = [cp for cp in checkpoint_list if cp.step_num == step_num]
                if step_checkpoints:
    ...
    def render_tree(self, state: WorkflowState, show_checkpoints: bool = True) -> str:
        ...
        # Header
        lines.append(f"{helpers.Colors.BOLD}Workflow: {state.change_id}{helpers.Colors.RESET}")
        ...
        lines.append(f"Progress: {completed_count}/{state.total_steps} steps ({progress_pct:.1f}%)")
        lines.append("")
        
        # Render each step
        for step_num in range(state.total_steps):
            step_info = self.get_step_info(step_num, state)
            lines.append(self._render_tree_step(step_info, step_num == state.total_steps - 1, show_checkpoints))
        ...
        if show_checkpoints and state.checkpoints:
            ...
            lines.append(f"{helpers.Colors.CYAN}Total Checkpoints: {total_checkpoints}{helpers.Colors.RESET}")
            if state.last_checkpoint_time:
                lines.append(f"Last Checkpoint: {state.last_checkpoint_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        return "\n".join(lines)
    
    def _render_tree_step(self, step: StepInfo, is_last: bool, show_checkpoints: bool) -> str:
    ...
    def render_timeline(self, state: WorkflowState) -> str:
        ...
        # Header
        lines.append(f"{helpers.Colors.BOLD}Workflow Timeline: {state.change_id}{helpers.Colors.RESET}")
        lines.append("")
        ...
        for step_num in range(state.total_steps):
            ...
            if step_info.status == 'completed':
                timeline_symbols.append(f"{helpers.Colors.GREEN}{self.SYMBOLS['completed']}{helpers.Colors.RESET}")
            elif step_info.status == 'current':
                timeline_symbols.append(f"{helpers.Colors.YELLOW}{self.SYMBOLS['current']}{helpers.Colors.RESET}")
            elif step_info.status == 'failed':
                timeline_symbols.append(f"{helpers.Colors.RED}{self.SYMBOLS['failed']}{helpers.Colors.RESET}")
            else:
                timeline_symbols.append(f"{helpers.Colors.GRAY}{self.SYMBOLS['pending']}{helpers.Colors.RESET}")
        
        # Join with connectors
        timeline = f" {helpers.Colors.GRAY}{self.SYMBOLS['connector']}{helpers.Colors.RESET} ".join(timeline_symbols)
        ...
        lines.append(f"{helpers.Colors.CYAN}{bar} {progress_pct:.1f}%{helpers.Colors.RESET}")
    ...
    def render_detailed(self, state: WorkflowState) -> str:
        ...
        # Header
        lines.append("=" * 70)
        lines.append(f"{helpers.Colors.BOLD}Workflow Detailed Report{helpers.Colors.RESET}")
        lines.append("=" * 70)
        lines.append("")
        ...
        lines.append(f"  Completed: {completed_count}/{state.total_steps} ({progress_pct:.1f}%)")
        lines.append(f"  Current: {state.current_step if state.current_step is not None else 'N/A'}")
        ...
        if state.completed_steps:
            lines.append(f"{helpers.Colors.GREEN}Completed Steps:{helpers.Colors.RESET}")
            for step_num in state.completed_steps:
                step_name = self.STEPS.get(step_num, f"Step {step_num}")
                checkpoint_marker = ""
                if step_num in state.checkpoints:
                    checkpoint_marker = f" {self.SYMBOLS['checkpoint']} {state.checkpoints[step_num]}"
                lines.append(f"  {self.SYMBOLS['completed']} {step_num:2d}: {step_name}{checkpoint_marker}")
            lines.append("")
        ...
        if state.current_step is not None:
            ...
            lines.append(f"  {self.SYMBOLS['current']} {state.current_step:2d}: {step_name}{checkpoint_marker}")
            lines.append("")
        
        # Pending steps
        pending_steps = [i for i in range(state.total_steps) 
        ...
        # Checkpoint summary
        if state.checkpoints:
            lines.append(f"{helpers.Colors.CYAN}Checkpoint Summary:{helpers.Colors.RESET}")
            total_checkpoints = sum(state.checkpoints.values())
            lines.append(f"  Total: {total_checkpoints}")
            if state.last_checkpoint_time:
                lines.append(f"  Last: {state.last_checkpoint_time.strftime('%Y-%m-%d %H:%M:%S')}")
            lines.append("")

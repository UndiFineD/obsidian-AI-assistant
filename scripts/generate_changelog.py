...
CHANGE_PROPOSALS_DIR = os.path.join(os.path.dirname(__file__), '..', 'openspec', 'changes')
...
TYPE_PATTERN = re.compile(r'update-(doc|spec|test|ci|security|release)-[\w-]+', re.IGNORECASE)
...
def main():
    parser = argparse.ArgumentParser(description='Generate CHANGELOG.md from OpenSpec proposals.')
    parser.add_argument('--output', type=str, default=DEFAULT_OUTPUT, help='Output file path')
    args = parser.parse_args()
    proposal_files = glob.glob(os.path.join(CHANGE_PROPOSALS_DIR, '**', 'proposal.md'), recursive=True)

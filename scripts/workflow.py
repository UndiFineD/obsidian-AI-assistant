...
def prompt_for_missing_info(change_id: str, title: Optional[str], owner: Optional[str], template: Optional[str] = None) -> tuple[str, str, str]:
    ...
    elif any(keyword in change_id.lower() for keyword in ["doc", "readme", "guide", "manual"]):
        suggested_template = "docs"
    elif any(keyword in change_id.lower() for keyword in ["fix", "bug", "defect", "issue"]):
        suggested_template = "bugfix"
    elif any(keyword in change_id.lower() for keyword in ["refactor", "cleanup", "restructure"]):
        suggested_template = "refactor"
    elif any(keyword in change_id.lower() for keyword in ["feature", "add", "new", "implement"]):
        suggested_template = "feature"

    try:
        print()
        print(f"{helpers.Colors.YELLOW}Press Enter to accept defaults or type a new value.{helpers.Colors.RESET}")
        t = input(f"Title [{derived_title}]: ").strip()
        o = input(f"Owner [{detected_owner}]: ").strip()
        print(f"{helpers.Colors.CYAN}Template options: feature, bugfix, docs, refactor, default{helpers.Colors.RESET}")
...
def validate_change(change_id: str):
    ...
    print(f"{helpers.Colors.CYAN}Validating change: {change_id}{helpers.Colors.RESET}\n")
...
def cleanup_checkpoints_cmd(change_id: str, keep_count: int):
    ...
    try:
        ...
        if removed > 0:
            helpers.write_success(f"Removed {removed} old checkpoint(s), kept {keep_count} most recent")
        else:
            helpers.write_info(f"No checkpoints to remove (have {len(checkpoint_manager.list_checkpoints())})")
...
def execute_step(
    ...
    if CHECKPOINT_AVAILABLE and enable_checkpoints and not dry_run:
        try:
            ...
            checkpoint_id = checkpoint_manager.create_checkpoint(
                step_num,
                step_name,
                notes=f"Before executing step {step_num}"
            )
        except Exception as e:
    ...
    if not step_module:
        ...
        helpers.write_info(
            f"Expected: {SCRIPT_DIR}/workflow-step{step_num:02d}.py"
        )
        return False
    ...
    try:
        ...
        if checkpoint_manager and checkpoint_id:
            ...
            helpers.write_info(f"  python workflow.py --change-id {change_path.name} --rollback {checkpoint_id}")
...
def run_single_step(
    ...
    if not change_path.exists():
        print(f"{helpers.Colors.CYAN}Creating new change: {change_id}{helpers.Colors.RESET}")
    ...
    success = execute_step(step_num, change_path, title, owner, dry_run, release_type, template, enable_checkpoints)
...
def run_interactive_workflow(
    ...
    print("═" * 55)
    print(f"{helpers.Colors.CYAN}  OpenSpec Workflow - Interactive Mode{helpers.Colors.RESET}")
    print("═" * 55)
    print()
    ...
    print(f"{helpers.Colors.WHITE}Dry Run    : {'Yes' if dry_run else 'No'}{helpers.Colors.RESET}")
    print()
    ...
    if start_step > 0 and start_step < 13:
        print(f"{helpers.Colors.YELLOW}Resuming from Step {start_step} (previous steps validated){helpers.Colors.RESET}\n")

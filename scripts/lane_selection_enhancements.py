...
class LaneAutoDetector:
    ...
    def _get_git_changes(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if result.returncode == 0:
                changes["added"] = result.stdout.strip().split("\n") if result.stdout else []
    ...
    def detect_lane(self) -> Tuple[LaneType, str]:
        ...
        for file_path in self.git_changes["all"]:
            ...
            if any(file_lower.endswith(ext) for ext in [".py", ".js", ".ts", ".jsx", ".tsx"]):
            ...
            elif any(file_lower.endswith(name) for name in [
        ...
        if code_files or test_files or config_files:
            ...
            return LaneType.STANDARD, "Unknown file types detected. Using standard lane."
    ...
    def is_suitable_for_lane(self, lane: LaneType) -> Tuple[bool, str]:
        ...
        """
        code_files = [f for f in self.git_changes["all"] if f and any(
            f.lower().endswith(ext) for ext in [".py", ".js", ".ts", ".jsx", ".tsx"]
        )]
        
        if lane == LaneType.DOCS:
            if code_files:
                return False, f"Code changes detected ({len(code_files)} files), docs lane not suitable"
...
class LaneErrorHandler:
    ...
    def get_error_message(cls, error_code: str, **kwargs) -> str:
        ...
        """
        error_info = cls.ERROR_REMEDIATION.get(error_code, {
    ...
    @classmethod
    def warn_code_in_docs_lane(cls, code_files: List[str], user_confirmed: bool = False) -> bool:
        """
...
class LaneRecommendationEngine:
    ...
    def recommend(self, detected_lane: LaneType) -> Tuple[LaneType, float]:
        ...
        if detected_stats.get("success_rate", 100) < 90:
            ...
            confidence = min(0.95, 0.7 + (detected_stats.get("success_rate", 100) - 90) / 100)
    ...
    def update_stats(self, lane: str, duration_seconds: float, success: bool):
        ...
        if lane not in self.stats:
            self.stats[lane] = {"usage_count": 0, "avg_duration": 0, "success_rate": 100}
...
class LanePerformanceTracker:
    ...
    def end_tracking(self, stages_executed: int, stages_skipped: int, 
                    quality_gates_passed: bool, parallelization_used: bool):

...
class PostDeploymentValidator:
            ...
            try:
                result = subprocess.run(
                    ["python", "scripts/workflow.py", 
                     "--change-id", f"test-docs-lane-{i}",
                     "--title", f"Timing Test {i+1}",
                     "--owner", "automation",
                     "--lane", "docs",
                     "--dry-run"],
                    capture_output=True,
    ...
    def post_4_validate_feature_usability(self) -> bool:
        ...
        for lane in lanes:
            ...
            try:
                result = subprocess.run(
                    ["python", "scripts/workflow.py",
                     "--change-id", f"test-{lane}-usability",
                     "--title", f"Usability Test",
                     "--owner", "automation",
                     "--lane", lane,
                     "--dry-run"],
                    capture_output=True,
    ...
    def run_full_validation(self, skip_timing: bool = False) -> Dict:
        ...
        validations = [
            ("POST-1", "Docs Lane Timing", self.post_1_validate_docs_lane_timing, not skip_timing),
            ("POST-2", "Quality Gates Reliability", self.post_2_validate_quality_gates_reliability, True),
            ("POST-3", "Documentation Accessibility", self.post_3_validate_documentation_accessibility, True),
            ("POST-4", "Feature Usability", self.post_4_validate_feature_usability, True),
            ("POST-5", "All Tests Passing", self.post_5_validate_all_tests_passing, True),
        ]
        ...
        status = "✅ ALL VALIDATIONS PASSED" if overall_success else "❌ SOME VALIDATIONS FAILED"
        print(f"\n  Status: {status}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-timing", action="store_true", help="Skip timing tests (faster)")
    parser.add_argument("--output", default="post_deployment_results.json", help="Output file")

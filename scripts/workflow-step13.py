#!/usr/bin/env python3
"""
OpenSpec Workflow Step 13: Pull Request Creation
...
def get_change_doc_info(change_path: Path) -> dict:
    ...
    try:
        ...
        title_match = re.search(r"^#\s*(?:Change\s+)?Proposal:\s*(.+)", content, re.MULTILINE)
        if title_match:
        ...
        specs_match = re.search(r"^-\s*\*\*Affected specs\*\*:\s*(.+)", content, re.MULTILINE)
        if specs_match:
            doc_info["affected_specs"] = specs_match.group(1).strip()
        
        files_match = re.search(r"^-\s*\*\*Affected files\*\*:\s*(.+)", content, re.MULTILINE)
        if files_match:
            doc_info["affected_files"] = files_match.group(1).strip()
        
        code_match = re.search(r"^-\s*\*\*Affected code\*\*:\s*(.+)", content, re.MULTILINE)
        if code_match:
...
def create_pr_with_gh(title: str, body: str, branch: str, base: str = "main") -> tuple:
    ...
    try:
        result = subprocess.run(
            ["gh", "pr", "create", "--base", base, "--title", title, "--body-file", tmp_path],
            capture_output=True,
            text=True,
            check=True
        )
...
def invoke_step13(change_path, dry_run=False, new_version=None, **_):
    """
    ...
    docs_base = f"openspec/archive/{change_id}" if is_archived else f"openspec/changes/{change_id}"
    ...
    if not gh_available:
        ...
        print(f"  https://github.com/UndiFineD/obsidian-AI-assistant/compare/main...{branch}?expand=1")
        print()
    ...
    if existing_pr["exists"]:
        print(f"Warning: PR already exists for branch '{branch}': #{existing_pr['number']}")
    ...
    try:
        ...
        if success:
            ...
            print(f"  https://github.com/UndiFineD/obsidian-AI-assistant/compare/main...{branch}?expand=1")
            print()
            print("Step 13 completed with warnings (PR creation pending)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python workflow-step13.py <change_path> [--dry-run] [--version=X.Y.Z]")
        sys.exit(1)
    ...
    success = invoke_step13(change_path, dry_run=dry_run, new_version=new_version)

...
class HookRegistry:
    ...
    def execute_hooks(self, stage_num: int, context: HookContext,
                     fail_fast: bool = True) -> List[HookResult]:
        """
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        successful = sum(1 for r in self.execution_history if r.status == HookStatus.SUCCESS)
        failed = sum(1 for r in self.execution_history if r.status == HookStatus.ERROR)
        warnings = sum(1 for r in self.execution_history if r.status == HookStatus.WARNING)

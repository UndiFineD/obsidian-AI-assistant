...
BREAKING CHANGE: {breaking_change}""" if "{breaking_change}" else """\
{type}({scope}): {subject}
...
{metrics}""",
    ...
    def get_template(cls, commit_type: CommitType) -> str:
        ...
        return cls.TEMPLATES.get(commit_type, """{type}({scope}): {subject}
...
class CommitMessageBuilder:
    ...
    def build_interactive(self) -> str:
        ...
        for i, commit_type in enumerate(CommitMessageTemplate.get_all_types(), 1):
            print(f"  {i}. {commit_type.value} - {self._get_type_description(commit_type)}")
    ...
    def validate(self, message: str) -> ValidationResult:
        ...
        if not match:
            ...
            if match.group('breaking'):
                ...
                has_breaking_info = any(
                    'BREAKING CHANGE' in line for line in lines[1:]
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for commit in self.commits:
            ...
            author_counts[commit.author] = (
                author_counts.get(commit.author, 0) + 1
            )
...
class CommitSigner:
    ...
    def verify_signature(self, commit_hash: str, repo_path: Optional[Path] = None) -> bool:
        """
        ...
        for pattern, rule in self.rules.items():
            if self._pattern_matches(branch_name, pattern):
                ...
                if rule.get('require_signed_commits'):
                    warnings.append(
                        f"Branch '{branch_name}' requires signed commits"
                    )

...
class ExecutionSnapshot:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        data = asdict(self)
        data['stages'] = [s.to_dict() if isinstance(s, StageExecution) else s 
...
class TimelineVisualizer:
    ...
    def visualize_stages(self, stages: List[StageExecution], 
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: Path, retention_days: int = 7, 
    ...
    def create_checkpoint(self, change_id: str, execution_id: str, 
...
class EnhancedStatusTracker:
    ...
    def __init__(self, change_id: str, lane: str = "standard",
    ...
    def mark_stage_complete(self, stage_num: int, 
        ...
        # Find stage
        stage = next((s for s in self.snapshot.stages if s.stage_num == stage_num),
                    None)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': len(completed) / len(self.snapshot.stages) * 100 
                           if self.snapshot.stages else 0,
        }
...
class ExecutionHistory:
    ...
    def get_execution_trends(self, change_id: str, 
        ...
        """
        files = sorted(self.history_dir.glob(f"{change_id}_*.json"),
                      reverse=True)[:limit]

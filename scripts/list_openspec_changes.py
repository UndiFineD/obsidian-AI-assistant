...
def parse_change_id(change_id: str) -> Dict[str, any]:
    ...
    return {"id": change_id, "date": None, "description": change_id, "has_date": False}
...
def render_ascii_burndown(
    burndown_data: List[tuple], width: int = 60, height: int = 15
) -> str:
    """
    ...
    x_label = (
        f"     {start_date}"
        + " " * (width - len(start_date) - len(end_date) - 5)
        + f"{end_date}"
    )
    ...
    lines.append(
        f"  Trend:   {'+' if counts[-1] > counts[0] else '-'}{abs(counts[-1] - counts[0])} changes"
    )
    lines.append("=" * (width + 10))
...
def print_change_list(
    changes: List[Dict], sort_by: str = "date", limit: Optional[int] = None
):
    ...
    if sort_by == "date":
        changes = sorted(
            changes,
            key=lambda c: c["date"] if c["date"] else datetime.min,
            reverse=True,
        )
    elif sort_by == "age":
        changes = sorted(
            changes,
            key=lambda c: c["age_days"] if c["age_days"] is not None else -1,
            reverse=True,
        )
    elif sort_by == "name":
...
def main():
    ...
    parser.add_argument("--limit", type=int, help="Limit number of changes displayed")
    parser.add_argument(
        "--stale-only",
        action="store_true",
    ...
    parser.add_argument(
        "--burndown", action="store_true", help="Show ASCII burndown chart"
    )
    parser.add_argument(
        "--burndown-days",
        type=int,
    ...
    try:
        ...
        changes = list_active_changes(
            changes_dir, exclude_archive=not args.include_archive
        )

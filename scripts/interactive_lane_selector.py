        ...
        try:
            ...
            for file_path in all_changed:
                ...
                if file_path.endswith(('.md', '.txt', '.rst', '.adoc')):
                    docs_changes += 1
                    file_types['docs'] += 1
                elif file_path.endswith(('.test.py', '.spec.py', 'test_', '_test.py', '/tests/')):
                    test_changes += 1
                    file_types['tests'] += 1
                elif file_path.endswith(('.yaml', '.yml', '.json', '.toml', '.cfg', '.conf', '.ini')):
                    config_changes += 1
                    file_types['config'] += 1
                else:
                    code_changes += 1
...
class LaneRecommender:
    ...
    def recommend(self) -> LaneRecommendation:
        ...
        if total <= 10 and (tests > 0 or code_pct > 0):
            ...
            confidence = RecommendationConfidence.HIGH if tests > 0 else RecommendationConfidence.MEDIUM
...
class InteractiveLaneSelector:
    ...
    def run(self, auto_confirm: bool = False, force_lane: Optional[str] = None,
            dry_run: bool = False, json_output: bool = False) -> ExecutionResult:
        """
        ...
        try:
            ...
            if json_output:
                console.print_json(data=result.to_dict()) if HAS_RICH else print(json.dumps(result.to_dict(), indent=2))
    ...
    def _prompt_lane_selection(self) -> LaneType:
        ...
        if HAS_RICH:
            ...
            console.print("  2. [yellow]STANDARD[/yellow] - Standard changes (recommended)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Interactive Workflow Lane Selector"
    )
    parser.add_argument(
        '--auto-confirm',
        action='store_true',

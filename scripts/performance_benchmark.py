    ...
    def _execute_workflow(self, lane: LaneType, timeout: Optional[int]) -> float:
        ...
        try:
            ...
            cmd = [
                ...
                "--lane", lane.value,
                "--change-id", f"benchmark-{lane.value}-{int(time.time())}"
            ]
            
            timeout_val = timeout or (300 if lane == LaneType.DOCS else 
                                     900 if lane == LaneType.STANDARD else 1200)
    ...
    def generate_csv_report(self, output_path: Optional[Path] = None):
        ...
        try:
            with open(output_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "Benchmark ID", "Lane", "Start Time", "Duration", 
                    "Iterations", "Status", "Exec Time Mean", "Exec Time P95",
                    "Memory Max", "CPU Mean"
                ])
...
def main():
    ...
    if args.action == "run":
        ...
        if violations:
            ...
            for v in violations:
                print(f"    - {v.metric}: {v.actual_value:.2f} > {v.threshold} ({v.severity})")

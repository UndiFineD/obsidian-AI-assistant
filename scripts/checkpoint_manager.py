...
class CheckpointManager:
    ...
    def __init__(self, change_path):
        ...
        """
        self.change_path = Path(change_path) if not isinstance(change_path, Path) else change_path
    ...
    def rollback_to_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        backup_id = self._generate_checkpoint_id(99)  # Special step number for rollback backups
...
def format_checkpoint_list(checkpoints: List[CheckpointMetadata]) -> str:
    ...
    for i, cp in enumerate(checkpoints, 1):
        ...
        lines.append(f"   Files: {len(cp.files_snapshot)} file(s) - {', '.join(cp.files_snapshot[:5])}")

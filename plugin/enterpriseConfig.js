...
class EnterpriseConfig {
    ...
    async initialize() {
        try {
            ...
            console.error('Failed to initialize enterprise config:', error);
        }
    }
    ...
    async loadConfiguration() {
        try {
            ...
            const configResponse = await this.backendClient.request('/admin/config', {
                method: 'GET'
            });
            ...
            if (this.enterpriseAuth.isAuthenticated()) {
                const tenantId = this.enterpriseAuth.getCurrentTenant().tenant_id;
                const tenantResponse = await this.backendClient.request(`/tenant/${tenantId}/config`, {
                    method: 'GET'
                });
                
                if (tenantResponse.ok) {
                    this.tenantConfig = tenantResponse.data;
                }
            }
    ...
    createConfigurationInterface(containerEl) {
        containerEl.empty();
    ...
    createConfigTabs(containerEl) {
        const tabsEl = containerEl.createEl('div', { cls: 'config-tabs' });
        ...
        tabs.forEach(tab => {
            ...
            tabEl.addEventListener('click', () => {
                this.switchConfigTab(tab.id, containerEl);
            });
        });
    }
    ...
    switchConfigTab(tabId, containerEl) {
        ...
        if (contentEl) {
            this.renderConfigContent(tabId, contentEl);
        }
    }

    renderConfigContent(tabId, contentEl) {
        contentEl.empty();
    ...
    renderGeneralSettings(contentEl) {
        ...
        this.createToggleSetting(featuresGroup, {
        ...
        this.createTextSetting(systemGroup, {
        ...
        this.createSelectSetting(systemGroup, {
        ...
        this.createNumberSetting(systemGroup, {
            key: 'session_timeout',
    ...
    renderSecuritySettings(contentEl) {
        ...
        this.createToggleSetting(authGroup, {
            key: 'require_mfa',
        ...
        this.createNumberSetting(authGroup, {
        ...
        this.createToggleSetting(accessGroup, {
            key: 'ip_restriction',
        ...
        this.createTextAreaSetting(accessGroup, {
            key: 'allowed_ips',
        ...
        this.createSelectSetting(encryptionGroup, {
        ...
        this.createToggleSetting(encryptionGroup, {
            key: 'encrypt_at_rest',
    ...
    renderComplianceSettings(contentEl) {
        const settingsEl = contentEl.createEl('div', { cls: 'settings-section' });
        ...
        this.createToggleSetting(gdprGroup, {
            key: 'gdpr_enabled',
        ...
        this.createSelectSetting(gdprGroup, {
        ...
        this.createToggleSetting(soc2Group, {
            key: 'soc2_enabled',
        ...
        this.createSelectSetting(soc2Group, {
        ...
        this.createToggleSetting(reportingGroup, {
        ...
        this.createSelectSetting(reportingGroup, {
    ...
    renderTenantSettings(contentEl) {
        if (!this.enterpriseAuth.isAuthenticated()) {
            contentEl.createEl('p', { 
                text: 'Please sign in to manage tenant settings',
                cls: 'error-message'
            });
            return;
        }
        ...
        this.createTextSetting(infoGroup, {
            key: 'tenant_name',
            label: 'Tenant Name',
        ...
        this.createTextAreaSetting(infoGroup, {
        ...
        this.createNumberSetting(limitsGroup, {
            key: 'max_users',
            label: 'Maximum Users',
    ...
    renderIntegrationSettings(contentEl) {
        const settingsEl = contentEl.createEl('div', { cls: 'settings-section' });
        ...
        providers.forEach(provider => {
            ...
            this.createToggleSetting(ssoGroup, {
                key: `sso_${providerKey}_enabled`,
        ...
        this.createTextSetting(apiGroup, {
            key: 'webhook_url',
            label: 'Webhook URL',
        ...
        this.createSelectSetting(apiGroup, {
            key: 'api_rate_limit',
    ...
    renderNotificationSettings(contentEl) {
        ...
        this.createToggleSetting(emailGroup, {
        ...
        this.createTextSetting(emailGroup, {
            key: 'smtp_server',
            label: 'SMTP Server',
        ...
        this.createToggleSetting(alertGroup, {
            key: 'security_alerts',
    ...
    createSettingGroup(containerEl, title) {
        const groupEl = containerEl.createEl('div', { cls: 'setting-group' });
        groupEl.createEl('h4', { text: title, cls: 'setting-group-title' });
        return groupEl;
    }

    createToggleSetting(containerEl, setting) {
        const settingEl = containerEl.createEl('div', { cls: 'setting-item toggle-setting' });
    ...
    createTextSetting(containerEl, setting) {
        const settingEl = containerEl.createEl('div', { cls: 'setting-item text-setting' });
    ...
    createTextAreaSetting(containerEl, setting) {
        const settingEl = containerEl.createEl('div', { cls: 'setting-item textarea-setting' });
    ...
    createNumberSetting(containerEl, setting) {
        const settingEl = containerEl.createEl('div', { cls: 'setting-item number-setting' });
    ...
    createSelectSetting(containerEl, setting) {
        const settingEl = containerEl.createEl('div', { cls: 'setting-item select-setting' });
    ...
    createSaveButton(containerEl, saveCallback) {
        const buttonContainer = containerEl.createEl('div', { cls: 'setting-buttons' });
    ...
    async saveGeneralSettings() {
        const settings = this.collectSettings('general');
        
        const response = await this.backendClient.request('/admin/config/general', {
            method: 'POST',
            data: settings
        });
    ...
    async saveSecuritySettings() {
        const settings = this.collectSettings('security');
        
        const response = await this.backendClient.request('/admin/config/security', {
            method: 'POST',
            data: settings
        });
    ...
    async saveComplianceSettings() {
        const settings = this.collectSettings('compliance');
        
        const response = await this.backendClient.request('/admin/config/compliance', {
            method: 'POST',
            data: settings
        });
        
        if (!response.ok) {
            throw new Error(response.message || 'Failed to save compliance settings');
        }
    ...
    async saveTenantSettings() {
        const settings = this.collectSettings('tenant');
        const tenantId = this.enterpriseAuth.getCurrentTenant().tenant_id;
        
        const response = await this.backendClient.request(`/tenant/${tenantId}/config`, {
            method: 'POST',
            data: settings
        });
        
        if (!response.ok) {
            throw new Error(response.message || 'Failed to save tenant settings');
        }
    ...
    async saveIntegrationSettings() {
        const settings = this.collectSettings('integration');
        
        const response = await this.backendClient.request('/admin/config/integrations', {
            method: 'POST',
            data: settings
        });
        
        if (!response.ok) {
            throw new Error(response.message || 'Failed to save integration settings');
        }
    ...
    async saveNotificationSettings() {
        const settings = this.collectSettings('notification');
        
        const response = await this.backendClient.request('/admin/config/notifications', {
            method: 'POST',
            data: settings
        });
    ...
    showError(containerEl, message) {
        let errorEl = containerEl.querySelector('.config-error');
        if (!errorEl) {
            errorEl = containerEl.createEl('div', { cls: 'config-error' });
        }

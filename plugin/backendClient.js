...
class BackendClient {
  ...
  async request(endpoint, { method = 'GET', data = null, headers = {}, retry = 0, signal = null } = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        if (retry < this.maxRetries && [502, 503, 504].includes(res.status)) {
      ...
      if (retry < this.maxRetries && err.name !== 'AbortError') {
  ...
  async post(endpoint, data, signal = null) {
    return this.request(endpoint, { method: 'POST', data, signal });
  }

  async put(endpoint, data, signal = null) {
    return this.request(endpoint, { method: 'PUT', data, signal });
  }
  ...
  startPolling(endpoint, intervalMs, callback, errorCallback = null) {

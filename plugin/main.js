...
function loadPluginConfigFile(app) { try {
        ...
        if(fs.existsSync(candidate)) { const raw = fs.readFileSync(candidate, "utf-8");
...
class AIModal extends Modal {
    ...
    async onOpen() { const { contentEl } = this;
    contentEl.empty();
    ...
    if(offline) { const reloadBtn = statusDiv.createEl("button", { text: "Reload" });
        ...
        try { await backendClient.post("/restart", {});
...
class ObsidianAIAssistant extends Plugin {
    ...
    if(external && typeof external === "object") { this.settings = Object.assign({}, this.settings, external);
        await this.saveSettings();
    }

    // Initialize enterprise features if available
    if(ENTERPRISE_AVAILABLE && this.settings.enterprise?.enabled) { await this.initializeEnterpriseFeatures();
    }
    ...
    if(this.enterpriseAuth && this.enterpriseAuth.hasRole('admin')) { this.addRibbonIcon("shield", "Enterprise Admin", () => { this.openEnterpriseAdmin();
        });
    }
    ...
    async initializeEnterpriseFeatures() { try {
        // Initialize enterprise authentication
        if(EnterpriseAuth) { this.enterpriseAuth = new EnterpriseAuth(this);
        ...
        if(EnterpriseAdminDashboard && this.enterpriseAuth) { this.enterpriseAdmin = new EnterpriseAdminDashboard(this);
        }

        // Initialize enterprise configuration
        if(EnterpriseConfig) { this.enterpriseConfig = new EnterpriseConfig(this);
    ...
    } catch(error) { console.error("Failed to initialize enterprise features:", error);
        new Notice("Failed to initialize enterprise features: " + error.message);
    }
    }

    onAuthenticationChanged(isAuthenticated, user, tenant) { try { if(isAuthenticated) { console.log(`User authenticated: ${ user.email } (${ tenant.name })`);
        ...
        if(user.roles && user.roles.includes('admin')) { this.addRibbonIcon("shield", "Enterprise Admin", () => { this.openEnterpriseAdmin();
            });
        }
    ...
    } catch(error) { console.error("Authentication change handler failed:", error);
    }
    }
    ...
    if(rightPane && rightPane.view) { rightPane.view.refresh();
    }
    }

    openEnterpriseAdmin() { try { if(!this.enterpriseAuth || !this.enterpriseAuth.isAuthenticated()) { new Notice("Please sign in to access admin features");
        return;
        }

        if(!this.enterpriseAuth.hasRole('admin')) { new Notice("Admin access required");
        return;
        }
    ...
    } catch(error) { console.error("Failed to open enterprise admin:", error);
    ...
    if(this.enterpriseAuth) { this.enterpriseAuth.destroy();
    }

    if(this.enterpriseAdmin) { this.enterpriseAdmin.destroy();
    }
    }
...
class EnterpriseAdminModal extends Modal {
    ...
    try { if(this.plugin.enterpriseAdmin) { await this.plugin.enterpriseAdmin.createDashboard(contentEl);
        } else { contentEl.createEl("p", { text: "Enterprise admin not available" });
        }
    } catch(error) { console.error("Failed to create admin dashboard:", error);
        contentEl.createEl("p", { text: "Failed to load admin dashboard: " + error.message,
        cls: "error-message"
        });
    }
    }
...
class AIAssistantSettingTab extends PluginSettingTab {
    ...
    new Setting(containerEl)
        ...
        tgl
            ...
            .onChange(async(v) => { this.plugin.settings.features = this.plugin.settings.features || {};
    ...
    if(ENTERPRISE_AVAILABLE) { containerEl.createEl("h3", { text: "Enterprise Features" });
        ...
        .addToggle((tgl) =>
            ...
            .onChange(async(v) => { this.plugin.settings.enterprise = this.plugin.settings.enterprise || {};
                this.plugin.settings.enterprise.enabled = v;
                await this.plugin.saveSettings();

                if(v && !this.plugin.enterpriseAuth) { await this.plugin.initializeEnterpriseFeatures();
                }
            })
        );
        ...
        if(this.plugin.enterpriseAuth && this.plugin.enterpriseAuth.isAuthenticated()) { const user = this.plugin.enterpriseAuth.getCurrentUser();
            ...
            const logoutBtn = authSection.createEl("button", { text: "Sign Out",
            cls: "mod-warning"
            });
            logoutBtn.onclick = async() => { await this.plugin.enterpriseAuth.logout();
            this.display(); // Refresh settings };

            // Admin dashboard button
            if(this.plugin.enterpriseAuth.hasRole('admin')) { const adminBtn = authSection.createEl("button", { text: "Open Admin Dashboard",
                cls: "mod-cta"
            });
            adminBtn.onclick = () => { this.plugin.openEnterpriseAdmin();
            };
            }

            // Configuration button
            const configBtn = authSection.createEl("button", { text: "Enterprise Configuration"
            });
            configBtn.onclick = () => { this.openEnterpriseConfig();
            };

        } else { authSection.createEl("h4", { text: "Enterprise Sign In" });
            authSection.createEl("p", { text: "Sign in with your organization's SSO provider to access enterprise features."
            });

            const loginBtn = authSection.createEl("button", { text: "Sign In",
                cls: "mod-cta"
            });
            loginBtn.onclick = () => { this.openEnterpriseLogin();
            };
        }
    ...
    } else { containerEl.createEl("h3", { text: "Enterprise Features" });
        containerEl.createEl("p", { text: "Enterprise features are not available in this build.",
        cls: "mod-warning"
        });
    }
    ...
    fetchBtn.onclick = async() => { try { const cfg = await backendClient.get("/api/config");
        new Notice("Config loaded: api_port=" + cfg.api_port);
        } catch(error) { new Notice("Failed to fetch backend config: " + error.message);
        }
    };

    const reloadBtn = controls.createEl("button", { text: "Reload Backend Config" });
    reloadBtn.onclick = async() => { try { await backendClient.post("/api/config/reload", {});
        new Notice("Backend config reloaded");
        } catch(error) { new Notice("Reload failed: " + error.message);
        }
    };

    const saveBtn = controls.createEl("button", { text: "Save to Backend" });
    saveBtn.onclick = async() => { try { const updates = { allow_network: !!(this.plugin.settings.features && this.plugin.settings.features.allowNetwork),
        };
        ...
        } catch(error) { new Notice("Failed to save/reload backend config: " + error.message);
        }
    };
    }
    ...
    async displayEnterpriseStatus(containerEl) {
        ...
        if(response.ok) { const status = response.data;

            if(status.enterprise_enabled) { containerEl.createEl("p", { text: "✅ Enterprise backend available",
                cls: "mod-success"
            });
            ...
            Object.entries(status.features).forEach(([feature, enabled]) => { const item = featuresList.createEl("li");
                item.textContent = `${ enabled ? '✅' : '❌'} ${ feature.replace(/_/g, ' ').toUpperCase()}`;
            });

            } else { containerEl.createEl("p", { text: "❌ Enterprise features not enabled on backend",
                cls: "mod-warning"
            });
            }
        } else { containerEl.createEl("p", { text: "⚠️ Cannot connect to enterprise backend",
                cls: "mod-warning"
                });
            }
        }
        } catch(error) {
            containerEl.createEl("p", { text: "❌ Enterprise backend unavailable",
                cls: "mod-warning"
            });
        }
    }
...
class EnterpriseLoginModal extends Modal {
    ...
    try { if(this.plugin.enterpriseAuth) { this.plugin.enterpriseAuth.createLoginInterface(contentEl);
        } else { contentEl.createEl("p", { text: "Enterprise authentication not available" });
        }
    } catch(error) { console.error("Failed to create login interface:", error);
        contentEl.createEl("p", { text: "Failed to load login interface: " + error.message,
        cls: "error-message"
        });
    }
    }
...
class EnterpriseConfigModal extends Modal {
    ...
    try { if(this.plugin.enterpriseConfig) { this.plugin.enterpriseConfig.createConfigurationInterface(contentEl);
        } else { contentEl.createEl("p", { text: "Enterprise configuration not available" });
        }
    } catch(error) { console.error("Failed to create config interface:", error);
        contentEl.createEl("p", { text: "Failed to load configuration interface: " + error.message,
        cls: "error-message"
        });
    }
    }

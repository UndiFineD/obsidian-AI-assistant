...
class ObsidianAIAssistant extends Plugin { async onload() { console.log('Loading Obsidian AI Assistant Plugin');
    ...
    onunload() { this.app.workspace.detachLeavesOfType(VIEW_TYPE_TASK_QUEUE);
    ...
    // View Activation
    // ----------------------------
    async activateTaskQueueView() { this.app.workspace.detachLeavesOfType(VIEW_TYPE_TASK_QUEUE);
    ...
    async activateAnalyticsView() { this.app.workspace.detachLeavesOfType(VIEW_TYPE_ANALYTICS);
    const leaf = this.app.workspace.getRightLeaf(false);
    ...
    // Analytics Tracking
    // ----------------------------
    trackQA(prompt, answer, model) { const entry = { timestamp: Date.now(),
        prompt,
        answer,
    };
    ...
    async saveAnalytics() { await this.saveData({ settings: this.settings, analytics: this.analytics });
    }
    ...
    async saveSettings() { await this.saveData({ settings: this.settings, analytics: this.analytics });
    }
}
...
// Settings Tab
// ----------------------------
class AssistantSettingTab extends PluginSettingTab { constructor(app, plugin) { super(app, plugin);
    this.plugin = plugin;
    }
    ...
    new Setting(containerEl)
        ...
        text
            ...
            .onChange(async(v) => { this.plugin.settings.backendUrl = v;
    ...
    new Setting(containerEl)
        ...
        text
            ...
            .onChange(async(v) => { this.plugin.settings.vaultPath = v;
            await this.plugin.saveSettings();
            })
        );
    ...
    new Setting(containerEl)
        ...
        toggle
            ...
            .onChange(async(v) => { this.plugin.settings.preferFastModel = v;
            await this.plugin.saveSettings();
            })
        );
    ...
    new Setting(containerEl)
        ...
        dd
            ...
            .onChange(async(v) => { this.plugin.settings.voiceMode = v;
            await this.plugin.saveSettings();
            })
        );
    }
